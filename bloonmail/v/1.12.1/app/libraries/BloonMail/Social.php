...
class Social
{
	...
	public function GoogleDisconnect($oAccount)
	{
		...
		if ($oAccount && $oGoogle)
		{
			...
			{
				$aData = \BloonMail\Utils::DecodeKeyValues($sEncodedeData);
	...
	public function TwitterDisconnect($oAccount)
	{
		...
		if ($oAccount && $oTwitter)
		{
			...
			{
				$aData = \BloonMail\Utils::DecodeKeyValues($sEncodedData);
				if (is_array($aData) && isset($aData['id']))
				{
					$this->oActions->StorageProvider()->Clear(null,
						\BloonMail\Providers\Storage\Enumerations\StorageType::NOBODY,
	...
	public function popupServiceResult($sTypeStr, $sLoginUrl, $bLogin, $iErrorCode)
	{
		...
		else
		{
			...
			$sConnectionFunc = 'rl_'.\md5(\BloonMail\Utils::GetConnectionToken()).'_'.$sTypeStr.$sCallBackType.'_service';
		...
		try
		{
			...
			if ($this->oHttp->HasQuery('error'))
			{
				$iErrorCode = ('access_denied' === $this->oHttp->GetQuery('error')) ?
					\BloonMail\Notifications::SocialGoogleLoginAccessDisable : \BloonMail\Notifications::UnknownError;
			}
			else if ($oGoogle)
			{
				...
				if (!$this->oHttp->HasQuery('code'))
				{
					$aParams = array(
						...
						'state' => ($bGmail ? '1' : '0').'|'.\BloonMail\Utils::GetConnectionToken().'|'.$this->oActions->GetSpecAuthToken(),
						'response_type' => 'code'
					);
...
//					}
				...
				else if (!empty($sState) && $sCheckToken === \BloonMail\Utils::GetConnectionToken())
				{
					if (!empty($sCheckAuth))
					{
						...
						if (!empty($aUserInfoResponse['result']['id']))
						{
							if ($bLogin)
							{
								...
								else
								{
									...
									$aUserData = \BloonMail\Utils::DecodeKeyValues($sUserData);
								}
								...
								{
									$iErrorCode = \BloonMail\Notifications::SocialGoogleLoginAccessDisable;
								}
							}
							...
							if ($oAccount && !$bGmail)
							{
								...
								$oSettings->SetConf('GoogleAccessToken', \BloonMail\Utils::EncodeKeyValues(array(
									'id' => $aUserInfoResponse['result']['id']
								)));
								...
								$this->oActions->StorageProvider()->Put(null,
									\BloonMail\Providers\Storage\Enumerations\StorageType::NOBODY,
									$this->GoogleUserLoginStorageKey($oGoogle, $aUserInfoResponse['result']['id']),
									\BloonMail\Utils::EncodeKeyValues($aUserData));
				...
				else if ($oAccessToken)
				{
					...
					if ($oAccount)
					{
						if ($mData && 0 < \strlen($mData))
						{
							...
							$oSettings->SetConf('FacebookAccessToken', \BloonMail\Utils::EncodeKeyValues(array('id' => $mData)));
					...
					else
					{
						...
						{
							$iErrorCode = \BloonMail\Notifications::SocialFacebookLoginAccessDisable;
						}
					}
				}
			}
		...
		try
		{
			...
			if ($oTwitter)
			{
				$sSessionKey = \implode('_', array('twitter',
					\md5($oTwitter->config['consumer_secret']), \md5(\BloonMail\Utils::GetConnectionToken()), 'AuthSessionData'));
						...
						if ($oAuth && !empty($oAuth['oauth_token']) && !empty($oAuth['oauth_token_secret']))
						{
							...
							if (200 === $iCode && isset($oTwitter->response['response']))
							{
								...
								if ($aAccessToken && isset($aAccessToken['oauth_token']) && !empty($aAccessToken['user_id']))
								{
									...
									$oSettings = $this->oActions->SettingsProvider()->Load($oAccount);
									$oSettings->SetConf('TwitterAccessToken', \BloonMail\Utils::EncodeKeyValues($aAccessToken));
						...
						if ($oAuth && !empty($oAuth['oauth_token']) && !empty($oAuth['oauth_token_secret']))
						{
							...
							if (200 === $iCode && isset($oTwitter->response['response']))
							{
								...
								if ($aAccessToken && isset($aAccessToken['oauth_token']) && !empty($aAccessToken['user_id']))
								{
									...
									{
										$iErrorCode = \BloonMail\Notifications::SocialTwitterLoginAccessDisable;
									}
	...
	/**
	 * @param \BloonMail\Model\Account|null $oAccount = null
	 * @param string $sRedirectUrl = ''
	 *
	 * @return \BloonMail\Common\BloonMailFacebookRedirectLoginHelper|null
	 */
		...
		)
		{
			...
			try
			{
				...
				$oFacebook = new \Facebook\Facebook(array(
					...
					'persistent_data_handler' => new \BloonMail\Common\FacebookBloonMailPersistentDataHandler(
						$oAccount, \BloonMail\Utils::GetConnectionToken(), $this->oActions->StorageProvider()
					)
				));
			}
			catch (\Exception $oException)
	...
	private function loginProcess(&$oAccount, $sEmail, $sPassword)
	{
		...
		try
		{
			...
			{
				$oAccount = null;
				$iErrorCode = \BloonMail\Notifications::AuthError;
			}
		}
