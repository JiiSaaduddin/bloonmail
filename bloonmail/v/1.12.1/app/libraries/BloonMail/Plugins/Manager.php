...
class Manager
{
	 ...
	 */
	public function __construct(\BloonMail\Actions $oActions)
	{
		$this->oLogger = null;
	 ...
	 * @param string $sName
	 *
	 * @return \BloonMail\Plugins\AbstractPlugin|null
	 */
	public function CreatePluginByName($sName)
	{
			...
			\file_exists(APP_PLUGINS_PATH.$sName.'/index.php'))
		{
			...
			if (\class_exists($sClassName))
			{
				...
				if ($oPlugin instanceof \BloonMail\Plugins\AbstractPlugin)
				{
					$oPlugin
						->SetName($sName)
						...
						->SetPluginConfig(new \BloonMail\Config\Plugin($sName, $oPlugin->ConfigMap()))
					;
				}
				else
				{
		...
		if ($this->bIsEnabled && isset($aAppData['Plugins']) && \is_array($aAppData['Plugins']))
		{
			...
			foreach ($this->aPlugins as $oPlugin)
			{
				if ($oPlugin)
				{
					...
					if (\is_array($aMap))
					{
						foreach ($aMap as /* @var $oPluginProperty \BloonMail\Plugins\Property */ $oPluginProperty)
						{
		...
		if ($this->bIsEnabled)
		{
			foreach ($this->aPlugins as $oPlugin)
			{
				if ($oPlugin->UseLangs())
				{
					...
					\BloonMail\Utils::ReadAndAddLang($sPath.'/langs/en.ini', $aLang);
					if ('en' !== $sLang)
					{
						\BloonMail\Utils::ReadAndAddLang($sPath.'/langs/'.$sLang.'.ini', $aLang);
					}
				}
			}
		}
