...
use BloonMail\Enumerations\UploadError;
use BloonMail\Enumerations\UploadClientError;
...
class Actions
{
	...
	/**
	 * @var \BloonMail\Providers\Files
	 */
	private $oFilesProvider;

	/**
	 * @var \BloonMail\Providers\Domain
	 */
	private $oDomainProvider;

	/**
	 * @var \BloonMail\Providers\Settings
	 */
	...
	/**
	 * @var \BloonMail\Providers\Filters
	 */
	private $oFiltersProvider;

	/**
	 * @var \BloonMail\Providers\AddressBook
	 */
	private $oAddressBookProvider;

	/**
	 * @var \BloonMail\Providers\Suggestions
	 */
	private $oSuggestionsProvider;

	/**
	 * @var \BloonMail\Providers\ChangePassword
	 */
	private $oChangePasswordProvider;

	/**
	 * @var \BloonMail\Providers\TwoFactorAuth
	 */
	private $oTwoFactorAuthProvider;

	/**
	 * @var \BloonMail\Providers\Prem
	 */
	private $oPremProvider;

	/**
	 * @var \BloonMail\Config\Application
	 */
	private $oConfig;
	...
	{
		$aAccountHash = \BloonMail\Utils::DecodeKeyValues($this->getLocalAuthToken());
		if (!empty($aAccountHash[0]) && 'token' === $aAccountHash[0] && \is_array($aAccountHash))
		{
			$aAccountHash[10] = \time() + $iLife;
			return \BloonMail\Utils::EncodeKeyValues($aAccountHash);
		}
	...
	public function Config()
	{
		...
		{
			$this->oConfig = new \BloonMail\Config\Application();
...
//			}
	...
	/**
	 * @param string $sName
	 * @param \BloonMail\Model\Account $oAccount = null
	 *
	 * @return mixed
	 */
	private function fabrica($sName, $oAccount = null)
	{
		...
		if (null === $mResult)
		{
			...
			{
				case 'files':
					// BloonMail\Providers\Files\IFiles
					$mResult = new \BloonMail\Providers\Files\FileStorage(APP_PRIVATE_DATA.'storage/files');
					break;
				case 'storage':
				case 'storage-local':
					// BloonMail\Providers\Storage\IStorage
					$mResult = new \BloonMail\Providers\Storage\FileStorage(
						APP_PRIVATE_DATA.'storage', 'storage-local' === $sName);
					break;
				case 'settings':
				case 'settings-local':
					// BloonMail\Providers\Settings\ISettings
					$mResult = new \BloonMail\Providers\Settings\DefaultSettings(
						$this->StorageProvider('settings-local' === $sName));
					break;
				case 'login':
					// \BloonMail\Providers\Login\LoginInterface
					$mResult = new \BloonMail\Providers\Login\DefaultLogin();
					break;
				case 'domain':
					// \BloonMail\Providers\Domain\DomainAdminInterface
					$mResult = new \BloonMail\Providers\Domain\DefaultDomain(APP_PRIVATE_DATA.'domains', $this->Cacher());
					break;
				case 'filters':
					// \BloonMail\Providers\Filters\FiltersInterface
					$mResult = new \BloonMail\Providers\Filters\SieveStorage(
						$this->Plugins(), $this->Config()
					);
					break;
				case 'address-book':
					// \BloonMail\Providers\AddressBook\AddressBookInterface
					...
					if ('sqlite' === $sDsnType)
					{
						$mResult = new \BloonMail\Providers\AddressBook\PdoAddressBook(
							'sqlite:'.APP_PRIVATE_DATA.'AddressBook.sqlite', '', '', 'sqlite');
					}
					else
					{
						$mResult = new \BloonMail\Providers\AddressBook\PdoAddressBook($sDsn, $sUser, $sPassword, $sDsnType);
					}
					break;
				case 'suggestions':
					...
					if (\is_array($mResult) && \BloonMail\Utils::IsOwnCloud() && $this->Config()->Get('labs', 'owncloud_suggestions', true))
					{
						// \BloonMail\Providers\Suggestions\ISuggestions
						$mResult[] = new \BloonMail\Providers\Suggestions\OwnCloudSuggestions();
					}

					break;
				case 'change-password':
					// \BloonMail\Providers\ChangePassword\ChangePasswordInterface
					break;
				case 'two-factor-auth':
					// \BloonMail\Providers\TwoFactorAuth\TwoFactorAuthInterface
					$mResult = new \BloonMail\Providers\TwoFactorAuth\GoogleTwoFactorAuth();
					break;
			}
		}
	...
	private function compileLogParams($sLine, $oAccount = null, $bUrlEncode = false, $aAdditionalParams = array())
	{
		...
		if (false !== \strpos($sLine, '{date:'))
		{
			...
			$sLine = \preg_replace_callback('/\{date:([^}]+)\}/', function ($aMatch) use ($sTimeOffset, $bUrlEncode) {
				return \BloonMail\Utils::UrlEncode(\MailSo\Log\Logger::DateHelper($aMatch[1], $sTimeOffset), $bUrlEncode);
			}, $sLine);
		...
		if (false !== \strpos($sLine, '{imap:') || false !== \strpos($sLine, '{smtp:'))
		{
			...
			if ($oAccount)
			{
				$sLine = \str_replace('{imap:login}', \BloonMail\Utils::UrlEncode($oAccount->IncLogin(), $bUrlEncode), $sLine);
				$sLine = \str_replace('{imap:host}', \BloonMail\Utils::UrlEncode($oAccount->DomainIncHost(), $bUrlEncode), $sLine);
				$sLine = \str_replace('{imap:port}', \BloonMail\Utils::UrlEncode($oAccount->DomainIncPort(), $bUrlEncode), $sLine);

				$sLine = \str_replace('{smtp:login}', \BloonMail\Utils::UrlEncode($oAccount->OutLogin(), $bUrlEncode), $sLine);
				$sLine = \str_replace('{smtp:host}', \BloonMail\Utils::UrlEncode($oAccount->DomainOutHost(), $bUrlEncode), $sLine);
				$sLine = \str_replace('{smtp:port}', \BloonMail\Utils::UrlEncode($oAccount->DomainOutPort(), $bUrlEncode), $sLine);
			}
		...
		if (false !== \strpos($sLine, '{request:'))
		{
			...
			{
				$sLine = \str_replace('{request:ip}', \BloonMail\Utils::UrlEncode($this->Http()->GetClientIp(
			...
			if (false !== \strpos($sLine, '{request:domain}'))
			{
				$sLine = \str_replace('{request:domain}',
					\BloonMail\Utils::UrlEncode($this->Http()->GetHost(false, true, true), $bUrlEncode), $sLine);
			}

			if (false !== \strpos($sLine, '{request:domain-clear}'))
			{
				$sLine = \str_replace('{request:domain-clear}',
					\BloonMail\Utils::UrlEncode(
		...
		if (false !== \strpos($sLine, '{user:'))
		{
			if (false !== \strpos($sLine, '{user:uid}'))
			{
				$sLine = \str_replace('{user:uid}',
					\BloonMail\Utils::UrlEncode(\base_convert(\sprintf('%u',
						\crc32(\md5(\BloonMail\Utils::GetConnectionToken()))), 10, 32), $bUrlEncode),
					$sLine
				);
			}
			...
			{
				$sLine = \str_replace('{user:ip}', \BloonMail\Utils::UrlEncode($this->Http()->GetClientIp(
					$this->Config()->Get('labs', 'http_client_ip_check_proxy', false)), $bUrlEncode), $sLine);
			}
			...
			if (\preg_match('/\{user:(email|login|domain)\}/i', $sLine))
			{
				...
				if ($oAccount)
				{
					...
					$sLine = \str_replace('{user:email}', \BloonMail\Utils::UrlEncode($sEmail, $bUrlEncode), $sLine);
					$sLine = \str_replace('{user:login}', \BloonMail\Utils::UrlEncode(
						\MailSo\Base\Utils::GetAccountNameFromEmail($sEmail), $bUrlEncode), $sLine);
					$sLine = \str_replace('{user:domain}', \BloonMail\Utils::UrlEncode(
						\MailSo\Base\Utils::GetDomainFromEmail($sEmail), $bUrlEncode), $sLine);
					$sLine = \str_replace('{user:domain-clear}', \BloonMail\Utils::UrlEncode(
	...
	{
		@\header('X-BloonMail-Action: Logout');
		\BloonMail\Utils::SetCookie(self::AUTH_SPEC_LOGOUT_TOKEN_KEY, \md5(APP_START_TIME), 0);
	}
	...
	public function SetAuthToken($oAccount)
	{
		if ($oAccount)
		{
			...
			\BloonMail\Utils::SetCookie(self::AUTH_SPEC_TOKEN_KEY, $sSpecAuthToken, 0);

			if ($oAccount->SignMe() && 0 < \strlen($oAccount->SignMeToken()))
			{
				\BloonMail\Utils::SetCookie(self::AUTH_SIGN_ME_TOKEN_KEY,
	...
	{
		$sResult = \BloonMail\Utils::GetCookie(self::AUTH_SPEC_TOKEN_KEY, '');
		if (0 < strlen($sResult))
		{
			\BloonMail\Utils::ClearCookie(self::AUTH_SPEC_TOKEN_KEY);
		}
	...
	{
		$sResult = \BloonMail\Utils::GetCookie(self::AUTH_SPEC_LOGOUT_TOKEN_KEY, '');
		if (0 < strlen($sResult))
		{
			\BloonMail\Utils::ClearCookie(self::AUTH_SPEC_LOGOUT_TOKEN_KEY);
		}
	...
	{
		$sResult = \BloonMail\Utils::GetCookie(self::AUTH_SPEC_LOGOUT_CUSTOM_MSG_KEY, '');
		if (0 < strlen($sResult))
		{
			\BloonMail\Utils::ClearCookie(self::AUTH_SPEC_LOGOUT_CUSTOM_MSG_KEY);
		}
	...
	{
		\BloonMail\Utils::SetCookie(self::AUTH_SPEC_LOGOUT_CUSTOM_MSG_KEY, $sMessage, 0);
	}
	...
	{
		\BloonMail\Utils::SetCookie(self::AUTH_ADMIN_TOKEN_KEY, $sToken, 0);
	}
	...
	{
		return \BloonMail\Utils::GetCookie(self::AUTH_ADMIN_TOKEN_KEY, '');
	}
	...
	{
		$aAdminHash = \BloonMail\Utils::DecodeKeyValuesQ($this->getAdminAuthToken());
		if (
			!empty($aAdminHash[0]) && !empty($aAdminHash[1]) && !empty($aAdminHash[2]) &&
			'token' === $aAdminHash[0] && \md5(APP_SALT) === $aAdminHash[1]
		)
		{
			$this->Cacher(null, true)->Delete(\BloonMail\KeyPathHelper::SessionAdminKey($aAdminHash[2]));
		}

		\BloonMail\Utils::ClearCookie(self::AUTH_ADMIN_TOKEN_KEY);
	}

	/**
	 * @param bool $bThrowExceptionOnFalse = false
	 *
	 * @return \BloonMail\Model\Account|bool
	...
	/**
	 * @return \BloonMail\Social
	 */
	public function Social()
	{
		if (null === $this->oSocial)
		{
			$this->oSocial = new \BloonMail\Social($this->Http(), $this);
		}
	...
	/**
	 * @return \BloonMail\Providers\Filters
	 */
	public function FiltersProvider()
	{
		if (null === $this->oFiltersProvider)
		{
			$this->oFiltersProvider = new \BloonMail\Providers\Filters(
	...
	/**
	 * @return \BloonMail\Providers\ChangePassword
	 */
	public function ChangePasswordProvider()
	{
		if (null === $this->oChangePasswordProvider)
		{
			$this->oChangePasswordProvider = new \BloonMail\Providers\ChangePassword(
	...
	/**
	 * @return \BloonMail\Providers\TwoFactorAuth
	 */
	public function TwoFactorAuthProvider()
	{
		if (null === $this->oTwoFactorAuthProvider)
		{
			$this->oTwoFactorAuthProvider = new \BloonMail\Providers\TwoFactorAuth(
	...
	/**
	 * @return \BloonMail\Providers\Prem
	 */
	public function PremProvider()
	{
		if (null === $this->oPremProvider)
		{
			if (\file_exists(APP_VERSION_ROOT_PATH.'app/libraries/BloonMail/Providers/Prem.php'))
			{
				$this->oPremProvider = new \BloonMail\Providers\Prem(
	 ...
	 *
	 * @return \BloonMail\Providers\Storage
	 */
	public function StorageProvider($bLocal = false)
	{
		if ($bLocal)
		{
			...
			{
				$this->oLocalStorageProvider = new \BloonMail\Providers\Storage(
		...
		else
		{
			...
			{
				$this->oStorageProvider = new \BloonMail\Providers\Storage(
	...
	/**
	 * @return \BloonMail\Providers\Settings
	 */
	public function SettingsProvider($bLocal = false)
	{
		if ($bLocal)
		{
			...
			{
				$this->oLocalSettingsProvider = new \BloonMail\Providers\Settings(
		...
		else
		{
			...
			{
				$this->oSettingsProvider = new \BloonMail\Providers\Settings(
	...
	/**
	 * @return \BloonMail\Providers\Files
	 */
	public function FilesProvider()
	{
		if (null === $this->oFilesProvider)
		{
			$this->oFilesProvider = new \BloonMail\Providers\Files(
				$this->fabrica('files'));
		}
	...
	/**
	 * @return \BloonMail\Providers\Domain
	 */
	public function DomainProvider()
	{
		if (null === $this->oDomainProvider)
		{
			$this->oDomainProvider = new \BloonMail\Providers\Domain(
	...
	/**
	 * @return \BloonMail\Providers\Suggestions
	 */
	public function SuggestionsProvider()
	{
		if (null === $this->oSuggestionsProvider)
		{
			$this->oSuggestionsProvider = new \BloonMail\Providers\Suggestions(
	 ...
	 *
	 * @return \BloonMail\Providers\AddressBook
	 */
	public function AddressBookProvider($oAccount = null, $bForceEnable = false)
	{
		...
		{
			$oDriver = null;
			if ($this->GetCapa(false, false, \BloonMail\Enumerations\Capa::CONTACTS, $oAccount))
			{
			...
			$this->oAddressBookProvider = new \BloonMail\Providers\AddressBook($oDriver);
	...
	/**
	 * @return \BloonMail\Plugins\Manager
	 */
	public function Plugins()
	{
		if (null === $this->oPlugins)
		{
			$this->oPlugins = new \BloonMail\Plugins\Manager($this);
			...
			if (!!$this->Config()->Get('logs', 'enable', false))
			{
				...
				if (!empty($sSessionFilter))
				{
					...
					if (empty($aSessionParts[0]) || empty($aSessionParts[1]) ||
						(string) $aSessionParts[1] !== (string) \BloonMail\Utils::GetCookie($aSessionParts[0], ''))
					{
						return $this->oLogger;
					}
				}
				...
				$this->oLogger->Write('['.
					...
					'][PDO:'.$sPdo.
					(\BloonMail\Utils::IsOwnCloud() ? '][cloud:true' : '').
	...
	private function getAdminToken()
	{
		...
		if (!$this->Cacher(null, true)->Set(\BloonMail\KeyPathHelper::SessionAdminKey($sRand), \time()))
		{
		...
		return '' === $sRand ? '' : \BloonMail\Utils::EncodeKeyValuesQ(array('token', \md5(APP_SALT), $sRand));
	}
	...
	public function IsAdminLoggined($bThrowExceptionOnFalse = true)
	{
		...
		if ($this->Config()->Get('security', 'allow_admin_panel', true))
		{
			...
			if (!empty($aAdminHash[0]) && !empty($aAdminHash[1]) && !empty($aAdminHash[2]) &&
				'token' === $aAdminHash[0] && \md5(APP_SALT) === $aAdminHash[1] &&
				'' !== $this->Cacher(null, true)->Get(\BloonMail\KeyPathHelper::SessionAdminKey($aAdminHash[2]), '')
			)
			{
				$bResult = true;
			}
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AuthError);
		}
	...
	public function SetMailtoRequest($sTo)
	{
		if (!empty($sTo))
		{
			\BloonMail\Utils::SetCookie(self::AUTH_MAILTO_TOKEN_KEY,
				\BloonMail\Utils::EncodeKeyValuesQ(array(
					'Time' => \microtime(true),
					'MailTo' => 'MailTo',
					'To' => $sTo
	 ...
	 *
	 * @return \BloonMail\Model\Account|null
	 */
	public function LoginProvide($sEmail, $sLogin, $sPassword, $sSignMeToken = '', $bThrowProvideException = false)
	{
		...
		if (0 < \strlen($sEmail) && 0 < \strlen($sLogin) && 0 < \strlen($sPassword))
		{
			...
			if ($oDomain instanceof \BloonMail\Model\Domain)
			{
				...
				{
					$oAccount = \BloonMail\Model\Account::NewInstance($sEmail, $sLogin, $sPassword, $oDomain, $sSignMeToken);
					$this->Plugins()->RunHook('filter.account', array(&$oAccount));

					if ($bThrowProvideException && !($oAccount instanceof \BloonMail\Model\Account))
					{
						throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AuthError);
					}
				}
				...
				{
					throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AccountNotAllowed);
				}
			}
			else if ($bThrowProvideException)
			{
				throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::DomainNotAllowed);
			}
		}
	...
	public function GetAccountFromCustomToken($sToken, $bThrowExceptionOnFalse = true, $bValidateShortToken = true, $bQ = false)
	{
		...
		if (!empty($sToken))
		{
			$aAccountHash = $bQ ? \BloonMail\Utils::DecodeKeyValuesQ($sToken) : \BloonMail\Utils::DecodeKeyValues($sToken);
			if (!empty($aAccountHash[0]) && 'token' === $aAccountHash[0] && // simple token validation
				...
				(!$bValidateShortToken || \BloonMail\Utils::GetShortToken() === $aAccountHash[7] ||  // check short token if needed
		...
		if ($bThrowExceptionOnFalse && !($oResult instanceof \BloonMail\Model\Account))
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AuthError);
		}
	...
	public function GetAccountFromSignMeToken()
	{
		...
		$sSignMeToken = \BloonMail\Utils::GetCookie(\BloonMail\Actions::AUTH_SIGN_ME_TOKEN_KEY, '');
		if (!empty($sSignMeToken))
		{
			$aTokenData = \BloonMail\Utils::DecodeKeyValuesQ($sSignMeToken);
			if (\is_array($aTokenData) && !empty($aTokenData['e']) && !empty($aTokenData['t']))
			{
				...
				{
					$aSignMeData = \BloonMail\Utils::DecodeKeyValuesQ($sTokenSettings);
		...
		{
			\BloonMail\Utils::ClearCookie(\BloonMail\Actions::AUTH_SIGN_ME_TOKEN_KEY);
		}
	...
	public function AppDataSystem($bAdmin = false, $bMobile = false, $bMobileDevice = false)
	{
		...
		if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::ATTACHMENTS_ACTIONS))
		{
			...
			if (\BloonMail\Utils::IsOwnCloudLoggedIn() && \class_exists('OCP\Files'))
			{
		...
		return \array_merge(array(
			...
			'webPath' => \BloonMail\Utils::WebPath(),
			'webVersionPath' => \BloonMail\Utils::WebVersionPath(),
			'token' => $oConfig->Get('security', 'csrf_protection', false) ? \BloonMail\Utils::GetCsrfToken() : '',
			...
			'jsHash' => \md5(\BloonMail\Utils::GetConnectionToken()),
			...
			'listPermanentFiltered' => '' !== \trim(\BloonMail\Api::Config()->Get('labs', 'imap_message_list_permanent_filter', '')),
...
*/
		...
		$aResult = array(
			...
			'DevEmail' => '',
			'DevPassword' => '',
			'Title' => $oConfig->Get('webmail', 'title', 'BloonMail Webmail'),
			'LoadingDescription' => $oConfig->Get('webmail', 'loading_description', 'BloonMail'),
			'LoadingDescriptionEsc' => 'BloonMail',
		...
		if ('' !== $aResult['LoadingDescription'] && 'BloonMail' !== $aResult['LoadingDescription'])
		{
		...
		if (!$bAdmin)
		{
			...
			if ($oAccount instanceof \BloonMail\Model\Account)
			{
				...
				{
					$sToken = \BloonMail\Utils::GetCookie(self::AUTH_MAILTO_TOKEN_KEY, null);
					if (null !== $sToken)
					{
						\BloonMail\Utils::ClearCookie(self::AUTH_MAILTO_TOKEN_KEY);

						$mMailToData = \BloonMail\Utils::DecodeKeyValuesQ($sToken);
			...
			{
				if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::TWO_FACTOR, $oAccount) &&
					$this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::TWO_FACTOR_FORCE, $oAccount) &&
...
//					|| \MailSo\Base\Utils::ValidateIP($aResult['AdminDomain']);
		...
		$aResult['SignMe'] = (string) $oConfig->Get('login', 'sign_me_auto', \BloonMail\Enumerations\SignMeType::DEFAILT_OFF);
		...
		$aResult['Layout'] = (int) $oConfig->Get('defaults', 'view_layout', \BloonMail\Enumerations\Layout::SIDE_PREVIEW);
		...
		if (!$bAdmin && $oAccount instanceof \BloonMail\Model\Account)
		{
			$aResult['ParentEmail'] = $oAccount->ParentEmail();

			$oSettingsLocal = $this->SettingsProvider(true)->Load($oAccount);

			if ($oSettingsLocal instanceof \BloonMail\Settings)
			{
//				if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::FOLDERS, $oAccount))
			...
			if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::SETTINGS, $oAccount))
			{
				if ($oSettings instanceof \BloonMail\Settings)
				{
					...
					if (!$this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::AUTOLOGOUT, $oAccount))
					{
						$aResult['AutoLogout'] = 0;
					}

					if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::USER_BACKGROUND, $oAccount))
					{
...
//						}
				...
				if ($oSettingsLocal instanceof \BloonMail\Settings)
				{
					...
					if ($this->GetCapa(false, $bMobile, \BloonMail\Enumerations\Capa::THEMES, $oAccount))
					{
		...
		if ($bMobile)
		{
			$aResult['Layout'] = \BloonMail\Enumerations\Layout::NO_PREVIW;
	...
	public function CheckMailConnection($oAccount, $bAuthLog = false)
	{
		...
		catch (\BloonMail\Exceptions\ClientException $oException)
		{
			throw $oException;
		}
		...
		catch (\MailSo\Imap\Exceptions\LoginBadCredentialsException $oException)
		{
			...
			{
				throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AuthError, $oException);
			}
		}
	 ...
	 *
	 * @return \BloonMail\Model\Account
	 * @throws \BloonMail\Exceptions\ClientException
	 */
		...
		$sAdditionalCode = '', $bAdditionalCodeSignMe = false, $bSkipTwoFactorAuth = false)
	{
		...
		if (false === \strpos($sEmail, '@'))
		{
			...
			if (false === \strpos($sEmail, '@') && !!$this->Config()->Get('login', 'determine_user_domain', false))
			{
				...
				while (0 < \count($aDomainParts) && 0 < $iLimit)
				{
					...
					if ($oDomain && $oDomain instanceof \BloonMail\Model\Domain)
					{
						$bAdded = true;
		...
		if (false === \strpos($sEmail, '@') || 0 === \strlen($sPassword))
		{
			...
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::InvalidInputArgument);
		}
		...
		if (!$bSkipTwoFactorAuth && $this->TwoFactorAuthProvider()->IsActive())
		{
			...
			{
				$sSecretHash = \md5(APP_SALT.$aData['Secret'].\BloonMail\Utils::Fingerprint());
				$sSecretCookieHash = \BloonMail\Utils::GetCookie(self::AUTH_TFA_SIGN_ME_TOKEN_KEY, '');
				...
				if (empty($sSecretCookieHash) || $sSecretHash !== $sSecretCookieHash)
				{
					...
					if (empty($sAdditionalCode))
					{
						...
						throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AccountTwoFactorAuthRequired);
					}
					else
					{
						...
						if (!$bUseBackupCode && !$this->TwoFactorAuthProvider()->VerifyCode($aData['Secret'], $sAdditionalCode))
						{
							...
							throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AccountTwoFactorAuthError);
						}

						if ($bAdditionalCodeSignMe)
						{
							\BloonMail\Utils::SetCookie(self::AUTH_TFA_SIGN_ME_TOKEN_KEY, $sSecretHash,
								\time() + 60 * 60 * 24 * 14);
						}
					}
				}
	...
	public function DoLogin()
	{
		...
		{
			if ($oException &&
				\BloonMail\Notifications::AccountTwoFactorAuthRequired === $oException->getCode())
			{
	...
	{
		if ($this->GetCapa(false, false, \BloonMail\Enumerations\Capa::ADDITIONAL_ACCOUNTS, $oAccount))
		{
	...
	public function GetTemplates($oAccount)
	{
		...
		if ($oAccount)
		{
			...
			if (\is_array($aData) && 0 < \count($aData))
			{
				...
				{
					$oItem = \BloonMail\Model\Template::NewInstance();
	 ...
	 * @param string $sID
	 *
	 * @return \BloonMail\Model\Identity
	 */
	...
	public function GetIdentities($oAccount)
	{
		$bAllowIdentities = $this->GetCapa(false, false,
			\BloonMail\Enumerations\Capa::IDENTITIES, $oAccount);
		...
		if ($oAccount)
		{
			...
			if (\is_array($aSubIdentities) && 0 < \count($aSubIdentities))
			{
				...
				{
					$oItem = \BloonMail\Model\Identity::NewInstance();
					$oItem->FromJSON($aItem);
			...
			if (!$bHasAccountIdentity)
			{
				\array_unshift($aIdentities,
					\BloonMail\Model\Identity::NewInstanceFromAccount($oAccount));
			}
	...
	{
		$bAllowIdentities = $this->GetCapa(false, false, \BloonMail\Enumerations\Capa::IDENTITIES, $oAccount);
	...
	public function DoFilters()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::FILTERS, $oAccount))
		{
	...
	public function DoFiltersSave()
	{
		...
		foreach ($aIncFilters as $aFilter)
		{
			if ($aFilter)
			{
				$oFilter = new \BloonMail\Providers\Filters\Classes\Filter();
	...
	public function DoAccountSetup()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AccountAlreadyExists);
		}
		else if (!$bNew && !isset($aAccounts[$sEmail]))
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::AccountDoesNotExist);
		}
	...
	{
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::ATTACHMENTS_ACTIONS))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
				...
				case 'zip':
					...
					if (\class_exists('ZipArchive'))
					{
						...
						if (!empty($sZipFileName))
						{
							...
							$oZip->setArchiveComment('BloonMail/'.APP_VERSION);
						...
						if (!$bError)
						{
							$mResult = array(
								'Files' => array(array(
									...
									'Hash' => \BloonMail\Utils::EncodeKeyValuesQ(array(
										'V' => APP_VERSION,
				...
				case 'dropbox':
					...
					foreach ($aData as $aItem)
					{
						$mResult['Files'][] = array(
							...
							'Hash' => \BloonMail\Utils::EncodeKeyValuesQ($aItem)
						);
					}
	...
	public function DoIdentityUpdate()
	{
		...
		$oIdentity = \BloonMail\Model\Identity::NewInstance();
	...
	public function DoIdentityDelete()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::IDENTITIES, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
		...
		if (empty($sId))
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::UnknownError);
		}
	...
	public function DoTemplateSetup()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::TEMPLATES, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}

		$oTemplate = \BloonMail\Model\Template::NewInstance();
	...
	public function DoAccountsAndIdentitiesSortOrder()
	{
		...
		return $this->DefaultResponse(__FUNCTION__, $this->StorageProvider()->Put($oAccount,
			\BloonMail\Providers\Storage\Enumerations\StorageType::CONFIG, 'accounts_identities_order',
			\json_encode(array(
	...
	public function DoLogout()
	{
		...
		if ($oAccount)
		{
			...
			if (!$oAccount->IsAdditionalAccount())
			{
				\BloonMail\Utils::ClearCookie(\BloonMail\Actions::AUTH_SPEC_TOKEN_KEY);
			}
		}
	...
	public function DoAppDelayStart()
	{
		...
		\BloonMail\Utils::UpdateConnectionToken();
		...
		$sTimers = $this->StorageProvider()->Get(null,
			\BloonMail\Providers\Storage\Enumerations\StorageType::NOBODY, 'Cache/Timers', '');
	...
	/**
	 * @param \BloonMail\Config\Application $oConfig
...
//		return $this->DefaultResponse(__FUNCTION__, false);
		...
		$this->setCapaFromParams($oConfig, 'CapaAdditionalAccounts', \BloonMail\Enumerations\Capa::ADDITIONAL_ACCOUNTS);
		$this->setCapaFromParams($oConfig, 'CapaIdentities', \BloonMail\Enumerations\Capa::IDENTITIES);
		$this->setCapaFromParams($oConfig, 'CapaTemplates', \BloonMail\Enumerations\Capa::TEMPLATES);
		$this->setCapaFromParams($oConfig, 'CapaTwoFactorAuth', \BloonMail\Enumerations\Capa::TWO_FACTOR);
		$this->setCapaFromParams($oConfig, 'CapaTwoFactorAuthForce', \BloonMail\Enumerations\Capa::TWO_FACTOR_FORCE);
		$this->setCapaFromParams($oConfig, 'CapaOpenPGP', \BloonMail\Enumerations\Capa::OPEN_PGP);
		$this->setCapaFromParams($oConfig, 'CapaGravatar', \BloonMail\Enumerations\Capa::GRAVATAR);
		$this->setCapaFromParams($oConfig, 'CapaThemes', \BloonMail\Enumerations\Capa::THEMES);
		$this->setCapaFromParams($oConfig, 'CapaUserBackground', \BloonMail\Enumerations\Capa::USER_BACKGROUND);
		$this->setCapaFromParams($oConfig, 'CapaAttachmentThumbnails', \BloonMail\Enumerations\Capa::ATTACHMENT_THUMBNAILS);
	...
	public function DoAdminLicensing()
	{
		...
		if ($oPremProvider && 2 < \strlen(APP_SITE))
		{
			...
			{
				$iErrorCode = \BloonMail\Notifications::LicensingServerIsUnavailable;
			}
		}

		if (0 < $iErrorCode && !$mResult)
		{
			throw new \BloonMail\Exceptions\ClientException($iErrorCode);
		}
	...
	public function DoAdminDomainSave()
	{
		...
		return $this->DefaultResponse(__FUNCTION__,
			$oDomain instanceof \BloonMail\Model\Domain ? $this->DomainProvider()->Save($oDomain) : false);
	}
	...
	private function rainLoopUpdatable()
	{
		return @file_exists(APP_INDEX_ROOT_PATH.'index.php') &&
			...
			@is_writable(APP_INDEX_ROOT_PATH.'bloonmail/') &&
	...
	private function getRepositoryDataByUrl($sRepo, &$bReal = false)
	{
		...
		$sCacheKey = \BloonMail\KeyPathHelper::RepositoryCacheFile($sRepo, $sRepoFile);
		...
		if ('' === $sRep || 0 === $iRepTime || time() - 3600 > $iRepTime)
		{
			...
			$sRep = '' !== $sRepo ? $oHttp->GetUrlAsString($sRepPath, 'BloonMail', $sContentType, $iCode, $this->Logger(), 10,
	...
	private function getCoreData(&$bReal)
	{
		...
		$sCacheKey = \BloonMail\KeyPathHelper::RepositoryCacheCore($sRepo);
		$sRep = $this->Cacher()->Get($sCacheKey);
		if ('' !== $sRep)
		{
			$iRepTime = $this->Cacher()->GetTimer($sCacheKey);
		...
		if ('' === $sRep || 0 === $iRepTime || time() - 3600 > $iRepTime)
		{
			...
			$sRep = '' !== $sRepo ? $oHttp->GetUrlAsString($sRepo, 'BloonMail', $sContentType, $iCode, $this->Logger(), 10,
				$this->Config()->Get('labs', 'curl_proxy', ''), $this->Config()->Get('labs', 'curl_proxy_auth', '')) : false;

			if (false !== $sRep)
			{
				$aRep = @\json_decode($sRep, true, 10);
				$bReal = \is_array($aRep) && 0 < \count($aRep) && isset($aRep['id']) && 'bloonmail' === $aRep['id'];
	...
	private function getRepositoryData(&$bReal, &$bBloonMailUpdatable)
	{
		$bBloonMailUpdatable = $this->rainLoopUpdatable();
	...
	public function DoAdminPackagesList()
	{
		...
		$bReal = false;
		$bBloonMailUpdatable = false;
		$aList = $this->getRepositoryData($bReal, $bBloonMailUpdatable);

		return $this->DefaultResponse(__FUNCTION__, array(
			 'Real' => $bReal,
			 'MainUpdatable' => $bBloonMailUpdatable,
			 'List' => $aList
		));
	}
	...
	public function DoAdminCoreData()
	{
		...
		$bBloonMailUpdatable = $this->rainLoopUpdatable();
		$bBloonMailAccess = $this->rainLoopCoreAccess();
		...
		return $this->DefaultResponse(__FUNCTION__, array(
			 'Real' => $bReal,
			 'Access' => $bBloonMailAccess,
			 'Updatable' => $bBloonMailUpdatable,
	...
	public function DoAdminUpdateCoreData()
	{
		...
		$aData = array();
		if ($bBloonMailUpdatable && $bBloonMailAccess)
		{
			$aData = $this->getCoreData($bReal);
		}
	...
	public function DoAdminPluginDisable()
	{
		...
		if (!$bDisable)
		{
			...
			if ($oPlugin && ($oPlugin instanceof \BloonMail\Plugins\AbstractPlugin))
			{
				$sValue = $oPlugin->Supported();
				if (0 < \strlen($sValue))
				{
					return $this->FalseResponse(__FUNCTION__, \BloonMail\Notifications::UnsupportedPluginPackage, $sValue);
				}
			}
			else
			{
				return $this->FalseResponse(__FUNCTION__, \BloonMail\Notifications::InvalidPluginPackage);
			}
		}
		...
		if (!empty($sName))
		{
			...
			if ($oPlugin)
			{
				...
				if (is_array($aMap) && 0 < count($aMap))
				{
					...
					{
						if ($oItem && ($oItem instanceof \BloonMail\Plugins\Property))
						{
							$aItem = $oItem->ToArray();
							$aItem[0] = $oConfig->Get('plugin', $oItem->Name(), '');
							if (\BloonMail\Enumerations\PluginPropertyType::PASSWORD === $oItem->Type())
							{
								$aItem[0] = APP_DUMMY;
							}
	...
	public function DoAdminPluginSettingsUpdate()
	{
			...
			if ($oPlugin)
			{
				...
				if (is_array($aMap) && 0 < count($aMap))
				{
					foreach ($aMap as $oItem)
					{
						...
						if (\BloonMail\Enumerations\PluginPropertyType::PASSWORD !== $oItem->Type() || APP_DUMMY !== $sValue)
						{
							$mResultValue = null;
							switch ($oItem->Type()) {
								case \BloonMail\Enumerations\PluginPropertyType::INT:
									$mResultValue  = (int) $sValue;
									break;
								case \BloonMail\Enumerations\PluginPropertyType::BOOL:
									$mResultValue  = '1' === (string) $sValue;
									break;
								case \BloonMail\Enumerations\PluginPropertyType::SELECTION:
									if (is_array($oItem->DefaultValue()) && in_array($sValue, $oItem->DefaultValue()))
									{
										$mResultValue = (string) $sValue;
									}
									break;
								case \BloonMail\Enumerations\PluginPropertyType::PASSWORD:
								case \BloonMail\Enumerations\PluginPropertyType::STRING:
								case \BloonMail\Enumerations\PluginPropertyType::STRING_TEXT:
									$mResultValue = (string) $sValue;
									break;
							}
		...
		if (!$mResult)
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantSavePluginSettings);
		}
	...
	{
		$oAccount = $this->getAccountFromToken();
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::SETTINGS, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
		...
		if ($this->GetCapa(false, false, \BloonMail\Enumerations\Capa::THEMES, $oAccount))
		{
		...
		$this->setSettingsFromParams($oSettings, 'Layout', 'int', function ($iValue) {
			return (int) (\in_array((int) $iValue, array(\BloonMail\Enumerations\Layout::NO_PREVIW,
				\BloonMail\Enumerations\Layout::SIDE_PREVIEW, \BloonMail\Enumerations\Layout::BOTTOM_PREVIEW)) ?
					$iValue : \BloonMail\Enumerations\Layout::SIDE_PREVIEW);
		});
	...
	public function DoFolders()
	{
		...
		$bUseFolders = $this->GetCapa(false, false, \BloonMail\Enumerations\Capa::FOLDERS, $oAccount);
	...
	public function DoFolderCreate()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::FOLDERS, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantCreateFolder, $oException);
		}
	...
	public function DoFolderSubscribe()
	{
		...
		catch (\Exception $oException)
		{
			if ($bSubscribe)
			{
				throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantSubscribeFolder, $oException);
			}
			else
			{
				throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantUnsubscribeFolder, $oException);
			}
		}
	...
	public function DoFolderRename()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantRenameFolder, $oException);
		}
	...
	public function DoFolderDelete()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantDeleteNonEmptyFolder, $oException);
		}
		catch (\Exception $oException)
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantDeleteFolder, $oException);
		}
	...
	public function DoFolderClear()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::MailServerError, $oException);
		}
	...
	{
//		\sleep(1);
//		throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantGetMessageList);
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantGetMessageList, $oException);
		}
	...
	private function buildMessage($oAccount, $bWithDraftInfo = true)
	{
		...
		{
			$oMessage->SetXMailer('BloonMail/'.APP_VERSION);
		}
	...
	public function DoSaveMessage()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::COMPOSER, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
		...
		&$rMessageStream, &$iMessageStreamSize, $bDsn = false, $bAddHiddenRcpt = true)
	{
		...
		if ($oRcpt && 0 < $oRcpt->Count())
		{
			...
			try
			{
				...
				$oSmtpClient = \MailSo\Smtp\SmtpClient::NewInstance()->SetLogger($this->Logger());
				$oSmtpClient->SetTimeOuts(10, (int) \BloonMail\Api::Config()->Get('labs', 'smtp_timeout', 60));
					...
					if (\MailSo\Base\Utils::FunctionExistsAndEnabled('mail'))
					{
						...
						if ($aToCollection && $oFrom)
						{
							...
							if (!empty($sRawBody))
							{
								...
								if (!$bR)
								{
									throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantSendMessage);
								}
							}
						}
					}
			...
			catch (\MailSo\Net\Exceptions\ConnectionException $oException)
			{
				...
				{
					throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::ClientViewError, $oException);
				}
				else
				{
					throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::ConnectionError, $oException);
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::InvalidRecipients);
		}
	}
	...
	public function DoSendMessage()
	{
			...
			if ($oMessage)
			{
				...
				if (false !== $iMessageStreamSize)
				{
					...
					if (0 < \strlen($sSentFolder))
					{
						...
						{
							throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantSaveMessage, $oException);
						}
					}
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantSendMessage, $oException);
		}
			...
			if ($oMessage)
			{
				...
				if (false !== $iMessageStreamSize)
				{
					...
					if (!empty($sReadReceiptFlag))
					{
						...
						$this->Cacher($oAccount)->Set(\BloonMail\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $sFolderFullName, $sUid), '1');
	...
	public function DoQuota()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::QUOTA, $oAccount))
		{
	...
	private function getContactsSyncData($oAccount)
	{
		...
		if (!empty($sData))
		{
			$mData = \BloonMail\Utils::DecodeKeyValues($sData);
			if (\is_array($mData))
			{
				$mResult = array(
	...
	public function DoSaveContactsSyncData()
	{
		...
		$bResult = $this->StorageProvider()->Put($oAccount,
			\BloonMail\Providers\Storage\Enumerations\StorageType::CONFIG,
			'contacts_sync',
			\BloonMail\Utils::EncodeKeyValues(array(
				'Enable' => $bEnabled,
				'User' => $sUser,
	...
	public function DoContactsSync()
	{
		...
		if (!$bResult)
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::ContactsSyncError);
		}
		...
		if ($sData)
		{
			...
			if (!empty($mData['BackupCodes']))
			{
				...
				return $this->StorageProvider()->Put($oAccount,
					\BloonMail\Providers\Storage\Enumerations\StorageType::CONFIG,
					'two_factor',
					\BloonMail\Utils::EncodeKeyValues($mData)
				);
			}
		}
	...
	public function DoGetTwoFactorInfo()
	{
		...
		if (!$this->TwoFactorAuthProvider()->IsActive() ||
			!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::TWO_FACTOR, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
...
//		));
	...
	public function DoContactSave()
	{
		...
		if ($oAddressBookProvider && $oAddressBookProvider->IsActive() && 0 < \strlen($sRequestUid))
		{
			...
			if (!$oContact)
			{
				$oContact = new \BloonMail\Providers\AddressBook\Classes\Contact();
				if (0 < \strlen($sUid))
				{
			...
			if (\is_array($aProperties))
			{
				foreach ($aProperties as $aItem)
				{
					...
					{
						$oProp = new \BloonMail\Providers\AddressBook\Classes\Property();
	...
	public function DoSuggestions()
	{
		...
		$aResult = \BloonMail\Utils::RemoveSuggestionDuplicates($aResult);
	...
	public function DoMessage()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantGetMessage, $oException);
		}
	...
	public function DoMessageDelete()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantDeleteMessage, $oException);
		}
	...
	public function DoMessageMove()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantMoveMessage, $oException);
		}
	...
	public function DoMessageCopy()
	{
		...
		{
			throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::CantCopyMessage, $oException);
		}
	...
	public function DoMessageUploadAttachments()
	{
		...
		try
		{
			...
			if (\is_array($aAttachments) && 0 < \count($aAttachments))
			{
				...
				{
					$aValues = \BloonMail\Utils::DecodeKeyValuesQ($sAttachment);
					if (\is_array($aValues))
					{
	...
	public function Upload()
	{
		...
		$iError = $this->GetActionParam('Error', \BloonMail\Enumerations\UploadError::UNKNOWN);
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile))
		{
			...
			{
				$iError = \BloonMail\Enumerations\UploadError::ON_SAVING;
			}
			else
			{
		...
		{
			$iClientError = \BloonMail\Enumerations\UploadClientError::NORMAL;
	...
	public function DoClearUserBackground()
	{
		...
		if (!$this->GetCapa(false, false, \BloonMail\Enumerations\Capa::USER_BACKGROUND, $oAccount))
		{
			return $this->FalseResponse(__FUNCTION__);
		}
	...
	public function UploadBackground()
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile))
		{
			...
			{
				$iError = \BloonMail\Enumerations\UploadError::FILE_TYPE;
			}
		}
	...
	public function RawFramedView()
	{
		...
		if ($oAccount)
		{
			...
			$aData = \BloonMail\Utils::DecodeKeyValuesQ($sRawKey);
	...
	public function Capa($bAdmin, $bMobile = false, $oAccount = null)
	{
		...
		{
			$aResult[] = \BloonMail\Enumerations\Capa::FOLDERS;

			if ($oConfig->Get('capa', 'messagelist_actions', true))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::MESSAGELIST_ACTIONS;

				if ($oConfig->Get('capa', 'dangerous_actions', true))
				{
					$aResult[] = \BloonMail\Enumerations\Capa::DANGEROUS_ACTIONS;
				}
			}
		}
		...
		{
			$aResult[] = \BloonMail\Enumerations\Capa::RELOAD;
		}

		if ($oConfig->Get('capa', 'quota', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::QUOTA;
		}

		if ($oConfig->Get('capa', 'settings', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::SETTINGS;

			if ($oConfig->Get('webmail', 'allow_additional_accounts', false))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::ADDITIONAL_ACCOUNTS;
			}

			if ($oConfig->Get('webmail', 'allow_additional_identities', false))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::IDENTITIES;
			}

			if ($oConfig->Get('capa', 'x-templates', true) && !$bMobile)
			{
				$aResult[] = \BloonMail\Enumerations\Capa::TEMPLATES;
			}

			if ($oConfig->Get('webmail', 'allow_themes', false) && !$bMobile)
			{
				$aResult[] = \BloonMail\Enumerations\Capa::THEMES;
			}

			if ($oConfig->Get('webmail', 'allow_user_background', false) && !$bMobile)
			{
				$aResult[] = \BloonMail\Enumerations\Capa::USER_BACKGROUND;
			}

			if ($oConfig->Get('security', 'openpgp', false) && !$bMobile)
			{
				$aResult[] = \BloonMail\Enumerations\Capa::OPEN_PGP;
			}

			if ($oConfig->Get('capa', 'filters', false))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::FILTERS;
				if ($bAdmin || ($oAccount && $oAccount->Domain()->UseSieve()))
				{
					$aResult[] = \BloonMail\Enumerations\Capa::SIEVE;
				}
			}
		}
		...
		{
			$aResult[] = \BloonMail\Enumerations\Capa::TWO_FACTOR;

			if ($oConfig->Get('security', 'force_two_factor_auth', false) &&
				($bAdmin || ($oAccount && !$oAccount->IsAdditionalAccount())))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::TWO_FACTOR_FORCE;
			}
		}

		if ($oConfig->Get('capa', 'help', true) && !$bMobile)
		{
			$aResult[] = \BloonMail\Enumerations\Capa::HELP;
		}

		if ($oConfig->Get('capa', 'attachments_actions', false))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::ATTACHMENTS_ACTIONS;
		}

		if ($oConfig->Get('capa', 'message_actions', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::MESSAGE_ACTIONS;
		}

		if ($oConfig->Get('capa', 'composer', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::COMPOSER;

			if ($oConfig->Get('capa', 'contacts', true))
			{
				$aResult[] = \BloonMail\Enumerations\Capa::CONTACTS;
			}
		}

		if ($oConfig->Get('capa', 'search', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::SEARCH;

			if ($oConfig->Get('capa', 'search_adv', true) && !$bMobile)
			{
				$aResult[] = \BloonMail\Enumerations\Capa::SEARCH_ADV;
			}
		}

		if ($oConfig->Get('labs', 'allow_gravatar', false))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::GRAVATAR;
		}

		if ($oConfig->Get('interface', 'show_attachment_thumbnail', true))
		{
			$aResult[] = \BloonMail\Enumerations\Capa::ATTACHMENT_THUMBNAILS;
		}

		if ($oConfig->Get('labs', 'allow_prefetch', false) && !$bMobile)
		{
			$aResult[] = \BloonMail\Enumerations\Capa::PREFETCH;
		}

		if (!\BloonMail\Utils::IsOwnCloud())
		{
			$aResult[] = \BloonMail\Enumerations\Capa::AUTOLOGOUT;
		}
	...
	public function RawPublic()
	{
		...
		if (!empty($sHash))
		{
			$sData = $this->StorageProvider()->Get(null,
				...
				\BloonMail\KeyPathHelper::PublicFile($sHash)
			);
		}
	...
	private function getDecodedRawKeyValue($sRawKey)
	{
		...
		if (!empty($sRawKey))
		{
			$aValues = \BloonMail\Utils::DecodeKeyValuesQ($sRawKey);
			if (is_array($aValues))
			{
				$bResult = $aValues;
			}
	...
	public function ProcessTemplate($sName, $sHtml)
	{
		...
		return \BloonMail\Utils::ClearHtmlOutput($sHtml);
	}
	...
	public function ExceptionResponse($sActionName, $oException)
	{
		...
		if ($oException instanceof \BloonMail\Exceptions\ClientException)
		{
			$iErrorCode = $oException->getCode();
			$sErrorMessage = null;

			if ($iErrorCode === \BloonMail\Notifications::ClientViewError)
			{
		...
		{
			$iErrorCode = \BloonMail\Notifications::UnknownError;
	...
	public function StaticI18N($sKey)
	{
		...
		if (null === $aLang)
		{
			...
			$aLang = array();
//			\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'app/i18n/langs.ini', $aLang);
//			\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'langs/'.$sLang.'.ini', $aLang);
			\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'app/localization/langs.yml', $aLang);
			\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'app/localization/webmail/'.$sLang.'.yml', $aLang);
	...
	public function StaticPath($sPath)
	{
		...
		$sResult = \BloonMail\Utils::WebStaticPath().$sPath;
	...
	protected function responseObject($mResponse, $sParent = '', $aParameters = array())
	{
		...
		if (\is_object($mResponse))
		{
			...
			$bThumb = $this->GetCapa(false, false, \BloonMail\Enumerations\Capa::ATTACHMENT_THUMBNAILS);
			...
			else if ('MailSo\Mail\Message' === $sClassName)
			{
				...
				$mResult['RequestHash'] = \BloonMail\Utils::EncodeKeyValuesQ(array(
					'V' => APP_VERSION,
					'Account' => $oAccount ? \md5($oAccount->Hash()) : '',
				...
				if ('Message' === $sParent)
				{
					...
					if (!!$this->Config()->Get('labs', 'use_local_proxy_for_external_images', false))
					{
						$fAdditionalExternalFilter = function ($sUrl) {
							return './?/ProxyExternal/'.\BloonMail\Utils::EncodeKeyValuesQ(array(
								'Rnd' => \md5(\microtime(true)),
								'Token' => \BloonMail\Utils::GetConnectionToken(),
								'Url' => $sUrl
							)).'/';
						};
					}
...
//					$mResult['Plain'] = 0 === \strlen($sPlain) ? '' : \MailSo\Base\HtmlUtils::ConvertPlainToHtml($sPlain);
					...
					if (0 < \strlen($mResult['ReadReceipt']) && !$mResult['IsReadReceipt'])
					{
						...
						if (0 < \strlen($mResult['ReadReceipt']) && '1' === $this->Cacher($oAccount)->Get(
							\BloonMail\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $mResult['Folder'], $mResult['Uid']), '0'))
						{
							$mResult['ReadReceipt'] = '';
						}
					}
			...
			else if ('BloonMail\Providers\AddressBook\Classes\Contact' === $sClassName)
			{
				$mResult = \array_merge($this->objectData($mResponse, $sParent, $aParameters), array(
					/* @var $mResponse \BloonMail\Providers\AddressBook\Classes\Contact */
			...
			else if ('BloonMail\Providers\AddressBook\Classes\Tag' === $sClassName)
			{
				$mResult = \array_merge($this->objectData($mResponse, $sParent, $aParameters), array(
					/* @var $mResponse \BloonMail\Providers\AddressBook\Classes\Tag */
					'IdContactTag' => $mResponse->IdContactTag,
					'Name' => \MailSo\Base\Utils::Utf8Clear($mResponse->Name),
					'ReadOnly' => $mResponse->ReadOnly
				));
			}
			else if ('BloonMail\Providers\AddressBook\Classes\Property' === $sClassName)
			{
				// Simple hack
				...
				$mResult = \array_merge($this->objectData($mResponse, $sParent, $aParameters), array(
					/* @var $mResponse \BloonMail\Providers\AddressBook\Classes\Property */
			...
			else if ('MailSo\Mail\Attachment' === $sClassName)
			{
				...
				$mResult['Download'] = \BloonMail\Utils::EncodeKeyValuesQ(array(
					'V' => APP_VERSION,
					'Account' => $oAccount ? \md5($oAccount->Hash()) : '',
					'Folder' => $mResult['Folder'],
					'Uid' => $mResult['Uid'],
