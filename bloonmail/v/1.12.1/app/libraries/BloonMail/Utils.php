...
class Utils
{
	...
	{
		$sKeyFile = APP_VERSION_ROOT_PATH.'app/resources/BloonMail.asc';
		...
		if (null === \BloonMail\Utils::$RSA)
		{
			...
			if (\class_exists('Crypt_RSA'))
			{
				...
				if (!empty($sPrivateKey))
				{
					...
					\BloonMail\Utils::$RSA = $oRsa;
				}
			}
		}

		return \BloonMail\Utils::$RSA;
	}
	...
	{
		if (!empty(\BloonMail\Utils::$RsaKey))
		{
			return \BloonMail\Utils::$RsaKey;
		}

		\BloonMail\Utils::$RsaKey = \file_exists(APP_PRIVATE_DATA.'rsa/private') ?
			\file_get_contents(APP_PRIVATE_DATA.'rsa/private') : '';

		\BloonMail\Utils::$RsaKey = \is_string(\BloonMail\Utils::$RsaKey) ? \BloonMail\Utils::$RsaKey : '';
	}
	...
	static public function EncryptStringRSA($sString, $sKey = '')
	{
		...
		$sPrivateKey = \BloonMail\Utils::RsaPrivateKey();
		if (!empty($sPrivateKey))
		{
...
//		{
//			return \BloonMail\Utils::EncryptStringRSA($sString,
//				$sKey.'Q'.\BloonMail\Utils::GetShortToken());
//		}
...
//		if (\MailSo\Base\Utils::FunctionExistsAndEnabled('openssl_pkey_get_private'))
//		{
//			return \BloonMail\Utils::DecryptStringRSA($sEncriptedString,
//				$sKey.'Q'.\BloonMail\Utils::GetShortToken());
//		}
	...
	static public function EncodeKeyValues(array $aValues, $sCustomKey = '')
	{
		return \MailSo\Base\Utils::UrlSafeBase64Encode(
			\BloonMail\Utils::EncryptString(@\serialize($aValues), \md5(APP_SALT.$sCustomKey)));
	}
	...
	{
		return \MailSo\Base\Utils::UrlSafeBase64Encode(
			\BloonMail\Utils::EncryptStringQ(
	...
	{
		$aResult = @\unserialize(
			\BloonMail\Utils::DecryptStringQ(
				\MailSo\Base\Utils::UrlSafeBase64Decode($sEncodedValues), \md5(APP_SALT.$sCustomKey)));
	...
	static public function GetConnectionToken()
	{
		...
		$sToken = \BloonMail\Utils::GetCookie($sKey, null);
		if (null === $sToken)
		{
			$sToken = \MailSo\Base\Utils::Md5Rand(APP_SALT);
			\BloonMail\Utils::SetCookie($sKey, $sToken, \time() + 60 * 60 * 24 * 30);
		}
	...
	{
		return \md5(empty($_SERVER['HTTP_USER_AGENT']) ? 'BloonMailFingerprint' : $_SERVER['HTTP_USER_AGENT']);
	}
	...
	static public function GetShortToken()
	{
		...
		{
			$sToken = \MailSo\Base\Utils::Md5Rand(APP_SALT);
			\BloonMail\Utils::SetCookie($sKey, $sToken, 0);
		}
	...
	static public function UpdateConnectionToken()
	{
		...
		$sToken = \BloonMail\Utils::GetCookie($sKey, '');
		if (!empty($sToken))
		{
			\BloonMail\Utils::SetCookie($sKey, $sToken, \time() + 60 * 60 * 24 * 30);
		}
	...
	public static function ReadAndAddLang($sFileName, &$aResultLang)
	{
		if (\file_exists($sFileName))
		{
			...
			{
				$aLang = \BloonMail\Utils::CustomParseIniFile($sFileName, true);
			}
...
//		return $sHtml;
	...
	public static function CompileTemplates(&$aList, $sDirName, $sNameSuffix = '')
	{
		...
		{
			$aFileList = \BloonMail\Utils::FolderFiles($sDirName, '.html');
	...
	{
		if (null === \BloonMail\Utils::$Cookies)
		{
			\BloonMail\Utils::$Cookies = is_array($_COOKIE) ? $_COOKIE : array();
		}

		return isset(\BloonMail\Utils::$Cookies[$sName]) ? \BloonMail\Utils::$Cookies[$sName] : $mDefault;
	}
	...
	public static function SetCookie($sName, $sValue = '', $iExpire = 0, $sPath = null, $sDomain = null, $bSecure = null, $bHttpOnly = true)
	{
		...
		if (null === $sPath)
		{
			$sPath = \BloonMail\Utils::$CookieDefaultPath;
			$sPath = $sPath && 0 < \strlen($sPath) ? $sPath : null;
		}

		if (null === $bSecure)
		{
			$bSecure = \BloonMail\Utils::$CookieDefaultSecure;
		}

		\BloonMail\Utils::$Cookies[$sName] = $sValue;
	...
	public static function ClearCookie($sName)
	{
		...
		unset(\BloonMail\Utils::$Cookies[$sName]);
	...
	{
		return isset($_ENV['JMDREVOLUTION_OWNCLOUD']) && $_ENV['JMDREVOLUTION_OWNCLOUD'] &&
			\class_exists('OC');
	}
	/**
	 * @return bool
	...
	public static function WebPath()
	{
		...
		if (\BloonMail\Utils::IsOwnCloud())
		{
			...
			{
				$sAppPath = \rtrim(\trim(\OC_App::getAppWebPath('bloonmail')), '\\/').'/app/';
			}

			if (empty($sAppPath))
			{
				$sUrl = \MailSo\Base\Http::SingletonInstance()->GetUrl();
				if ($sUrl && \preg_match('/\/index\.php\/apps\/bloonmail/', $sUrl))
				{
					$sAppPath = \preg_replace('/\/index\.php\/apps\/bloonmail.+$/',
						'/apps/bloonmail/app/', $sUrl);
				}
			}
		}
	...
	{
		return self::WebPath().'bloonmail/v/'.APP_VERSION.'/';
	}
...
//		}
	...
	public static function CustomBaseConvert($sNumberInput, $sFromBaseInput = '0123456789', $sToBaseInput = '0123456789')
	{
		...
		{
			$sBase10 = \BloonMail\Utils::CustomBaseConvert($sNumberInput, $sFromBaseInput, '0123456789');
		}
		else
		{
