...
class FileStorage implements \BloonMail\Providers\Storage\IStorage
{
	/**
	 * @var string
	 */
	...
	public function DeleteStorage($oAccount)
	{
		$sPath = $this->generateFileName($oAccount,
			\BloonMail\Providers\Storage\Enumerations\StorageType::USER, 'xxx', false, true);
		...
		$sPath = $this->generateFileName($oAccount,
			\BloonMail\Providers\Storage\Enumerations\StorageType::CONFIG, 'xxx', false, true);
	...
	public function generateFileName($mAccount, $iStorageType, $sKey, $bMkDir = false, $bForDeleteAction = false)
	{
		if (null === $mAccount)
		{
			$iStorageType = \BloonMail\Providers\Storage\Enumerations\StorageType::NOBODY;
		}

		$sEmail = $sSubEmail = '';
		if ($mAccount instanceof \BloonMail\Model\Account)
		{
		...
		{
			default:
			case \BloonMail\Providers\Storage\Enumerations\StorageType::USER:
			case \BloonMail\Providers\Storage\Enumerations\StorageType::NOBODY:
				$sTypePath = 'data';
				$sKeyPath = \md5($sKey);
				$sKeyPath = \substr($sKeyPath, 0, 2).'/'.$sKeyPath;
				break;
			case \BloonMail\Providers\Storage\Enumerations\StorageType::CONFIG:
				$sTypePath = 'cfg';
		...
		if ($bMkDir && !$bForDeleteAction && !empty($sFilePath) && !@\is_dir(\dirname($sFilePath)))
		{
			...
			{
				throw new \BloonMail\Exceptions\Exception('Can\'t make storage directory "'.$sFilePath.'"');
			}
		}
