...
use \BloonMail\Providers\AddressBook\Enumerations\PropertyType as PropertyType;

class AddressBook extends \BloonMail\Providers\AbstractProvider
{
	/**
	 * @var \BloonMail\Providers\AddressBook\AddressBookInterface
	 */
	private $oDriver;

	/**
	 * @param \BloonMail\Providers\AddressBook\Interface $oDriver
	 *
	 * @return void
	 */
	...
	{
		$this->oDriver = null;
		if ($oDriver instanceof \BloonMail\Providers\AddressBook\AddressBookInterface)
		{
	...
	{
		\sleep(1);
		return $this->oDriver instanceof \BloonMail\Providers\AddressBook\AddressBookInterface ?
	...
	public function ImportVcfFile($sEmail, $sVcfData)
	{
		...
		if (\class_exists('SabreForBloonMail\DAV\Client') && $this->IsActive() && \is_string($sVcfData))
		{
			...
			try
			{
				$oVCardSplitter = new \SabreForBloonMail\VObject\Splitter\VCard($sVcfData);
			}
			...
			if ($oVCardSplitter)
			{
				...
				{
					if ($oVCard instanceof \SabreForBloonMail\VObject\Component\VCard)
					{
						\MailSo\Base\Utils::ResetTimeLimit();

						if (empty($oVCard->UID))
						{
							$oVCard->UID = \SabreForBloonMail\DAV\UUIDUtil::getUUID();
						}
