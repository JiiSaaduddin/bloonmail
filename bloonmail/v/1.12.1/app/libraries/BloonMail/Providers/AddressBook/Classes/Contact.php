...
namespace BloonMail\Providers\AddressBook\Classes;

use
	BloonMail\Providers\AddressBook\Enumerations\PropertyType,
	BloonMail\Providers\AddressBook\Classes\Property
;
...
class Contact
{
	...
	public function PopulateDisplayAndFullNameValue($bForceFullNameReplace = false)
	{
		...
		foreach ($this->Properties as /* @var $oProperty \BloonMail\Providers\AddressBook\Classes\Property */ &$oProperty)
		{
			if ($oProperty)
			{
		...
		if (!$oFullNameProperty)
		{
			$this->Properties[] = new \BloonMail\Providers\AddressBook\Classes\Property(PropertyType::FULLNAME, $this->Display);
		}
	}
	...
	{
		$this->IdContactStr = \class_exists('SabreForBloonMail\DAV\Client') ?
			\SabreForBloonMail\DAV\UUIDUtil::getUUID() : \MailSo\Base\Utils::Md5Rand();
	}
	...
	public function ToVCard($sPreVCard = '', $oLogger = null)
	{
		...
		if (0 < \strlen($sPreVCard))
		{
			try
			{
				$oVCard = \SabreForBloonMail\VObject\Reader::read($sPreVCard);
			}
...
//		}
		...
		if (!$oVCard)
		{
			$oVCard = new \SabreForBloonMail\VObject\Component\VCard();
		}

		$oVCard->VERSION = '3.0';
		$oVCard->PRODID = '-//BloonMail//'.APP_VERSION.'//EN';
	...
	public function PopulateByVCard($sUid, $sVCard, $sEtag = '', $oLogger = null)
	{
		...
		try
		{
			$oVCard = \SabreForBloonMail\VObject\Reader::read($sVCard);
		}
		catch (\Exception $oExc)
		{
			if ($oLogger)
...
//		}
		...
		if ($oVCard)
		{
			...
			$sUidValue = $oVCard->UID ? (string) $oVCard->UID : \SabreForBloonMail\DAV\UUIDUtil::getUUID();
