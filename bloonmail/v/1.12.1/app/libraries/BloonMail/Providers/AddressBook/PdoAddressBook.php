...
use \BloonMail\Providers\AddressBook\Enumerations\PropertyType;

class PdoAddressBook
	extends \BloonMail\Common\PdoAbstract
	implements \BloonMail\Providers\AddressBook\AddressBookInterface
{
	/**
	 * @var string
	 */
	...
	{
		return !!$this->prepareAndExecute('DELETE FROM bloonmail_ab_contacts WHERE id_user = :id_user AND deleted = 1', array(
	...
	{
		return !!$this->prepareAndExecute('UPDATE bloonmail_ab_contacts SET changed = :changed, etag = :etag '.
	...
	{
		$aResult = array();
		$oStmt = $this->prepareAndExecute('SELECT id_contact, id_contact_str, changed, deleted, etag FROM bloonmail_ab_contacts WHERE id_user = :id_user', array(
			':id_user' => array($iUserID, \PDO::PARAM_INT)
		));
...
//			}
	...
	/**
	 * @param \SabreForBloonMail\DAV\Client $oClient
	 * @param string $sPath
	 *
	 * @return bool
	 */
			...
			foreach ($aResponse as $sKey => $aItem)
			{
				...
				if (!empty($sKey))
				{
					...
					if (empty($sNextPath))
					{
						...
						/* @var $oResourceType \SabreForBloonMail\DAV\Property\ResourceType */
...
//			$this->oLogger->WriteDump($aResponse);
	...
	public function getDavClientFromUrl($sUrl, $sUser, $sPassword, $sProxy = '')
	{
		...
		$oClient = new \SabreForBloonMail\DAV\Client($aSettings);
	...
	{
		if (!\class_exists('SabreForBloonMail\DAV\Client'))
		{
			return false;
		}
...
//						$this->oLogger->WriteDump($sExsistensBody);
	...
	/**
	 * @param string $sEmail
	 * @param \BloonMail\Providers\AddressBook\Classes\Contact $oContact
	...
	public function ContactSave($sEmail, &$oContact, $bSyncDb = true)
	{
		...
		try
		{
			...
			if ($bUpdate)
			{
				...
				$sSql = 'UPDATE bloonmail_ab_contacts SET id_contact_str = :id_contact_str, display = :display, changed = :changed, etag = :etag '.
				...
				$this->prepareAndExecute(
					'DELETE FROM bloonmail_ab_properties WHERE id_user = :id_user AND id_contact = :id_contact',
					array(
						':id_user' => array($iUserID, \PDO::PARAM_INT),
						':id_contact' => array($iIdContact, \PDO::PARAM_INT)
					)
			...
			{
				$sSql = 'INSERT INTO bloonmail_ab_contacts '.
				...
				$sLast = $this->lastInsertId('bloonmail_ab_contacts', 'id_contact');
			...
			{
				$aParams = array();
				foreach ($oContact->Properties as /* @var $oProp \BloonMail\Providers\AddressBook\Classes\Property */ $oProp)
				{
					$iFreq = $oProp->Frec;
				...
				if (0 < \count($aParams))
				{
					$sSql = 'INSERT INTO bloonmail_ab_properties '.
	...
	public function DeleteContacts($sEmail, $aContactIds, $bSyncDb = true)
	{
		...
		$this->prepareAndExecute('DELETE FROM bloonmail_ab_properties WHERE id_user = :id_user AND id_contact IN ('.$sIDs.')', $aParams);
		...
		$this->prepareAndExecute('UPDATE bloonmail_ab_contacts SET deleted = 1, changed = :changed '.
	...
	public function DeleteAllContacts($sEmail, $bSyncDb = true)
	{
		...
		$this->prepareAndExecute('DELETE FROM bloonmail_ab_properties WHERE id_user = :id_user', $aParams);
		$this->prepareAndExecute('DELETE FROM bloonmail_ab_contacts WHERE id_user = :id_user', $aParams);
	...
	public function GetContacts($sEmail, $iOffset = 0, $iLimit = 20, $sSearch = '', &$iResultCount = 0)
	{
		...
		if (0 < \strlen($sSearch))
		{
			...
			$sSql = 'SELECT id_user, id_prop, id_contact FROM bloonmail_ab_properties '.
...
(0 < \strlen($sCustomSearch) ? ' OR (prop_type = '.PropertyType::PHONE.' AND prop_value_custom <> \'\' AND prop_value_custom LIKE :search_custom_phone)' : '').
		...
		{
			$sSql = 'SELECT COUNT(DISTINCT id_contact) as contact_count FROM bloonmail_ab_properties '.
		...
		if (0 < $iCount)
		{
			$sSql = 'SELECT * FROM bloonmail_ab_contacts WHERE deleted = 0 AND id_user = :id_user';
			...
			if ($oStmt)
			{
				...
				if (0 < count($aIdContacts))
				{
					...
					$sSql = 'SELECT * FROM bloonmail_ab_properties WHERE id_contact IN ('.\implode(',', $aIdContacts).')';
						...
						if (\is_array($aFetch) && 0 < \count($aFetch))
						{
							foreach ($aFetch as $aItem)
							{
								if ($aItem && isset($aItem['id_prop'], $aItem['id_contact'], $aItem['prop_type'], $aItem['prop_value']))
								{
									...
									{
										$oProperty = new \BloonMail\Providers\AddressBook\Classes\Property();
	 ...
	 *
	 * @return \BloonMail\Providers\AddressBook\Classes\Contact|null
	 */
	public function GetContactByID($sEmail, $mID, $bIsStrID = false)
	{
		...
		if ($oStmt)
		{
			...
			if (0 < $iIdContact && $oContact)
			{
				...
				$sSql = 'SELECT * FROM bloonmail_ab_properties WHERE id_contact = '.$iIdContact;
				$oStmt = $this->prepareAndExecute($sSql);
	...
	public function GetSuggestions($sEmail, $sSearch, $iLimit = 20)
	{
		...
		$sSql = 'SELECT id_contact, id_prop, prop_type, prop_value FROM bloonmail_ab_properties '.
...
(0 < \strlen($sLowerSearch) ? ' OR (prop_value_lower <> \'\' AND prop_value_lower LIKE :search_lower ESCAPE \'=\')' : '').
		...
		if ($oStmt)
		{
			...
			if (0 < count($aIdContacts))
			{
				...
				$sSql = 'SELECT id_prop, id_contact, prop_type, prop_value FROM bloonmail_ab_properties '.
	...
	public function IncFrec($sEmail, $aEmails, $bCreateAuto = true)
	{
		...
		if ($bCreateAuto)
		{
			$sSql = 'SELECT prop_value FROM bloonmail_ab_properties WHERE id_user = :id_user AND prop_type = :prop_type';
		...
		if (0 < \count($aEmailsToCreate))
		{
			...
			foreach ($aEmailsToCreate as $oEmail)
			{
				...
				{
					$oPropEmail = new \BloonMail\Providers\AddressBook\Classes\Property();
					$oPropEmail->Type = \BloonMail\Providers\AddressBook\Enumerations\PropertyType::EMAIl;
				...
				if ('' !== \trim($oEmail->GetDisplayName()))
				{
					...
					if (0 < \strlen($sFirst))
					{
						$oPropName = new \BloonMail\Providers\AddressBook\Classes\Property();
						$oPropName->Type = \BloonMail\Providers\AddressBook\Enumerations\PropertyType::FIRST_NAME;
					...
					{
						$oPropName = new \BloonMail\Providers\AddressBook\Classes\Property();
						$oPropName->Type = \BloonMail\Providers\AddressBook\Enumerations\PropertyType::LAST_NAME;
		...
		$sSql = 'UPDATE bloonmail_ab_properties SET prop_frec = prop_frec + 1 WHERE id_user = :id_user AND prop_type = :prop_type';
...
CREATE TABLE IF NOT EXISTS bloonmail_ab_contacts (
	...
	INDEX id_user_bloonmail_ab_contacts_index (id_user)

)/*!40000 ENGINE=INNODB *//*!40101 CHARACTER SET utf8 COLLATE utf8_general_ci */;

CREATE TABLE IF NOT EXISTS bloonmail_ab_properties (
	...
	PRIMARY KEY(id_prop),
	INDEX id_user_bloonmail_ab_properties_index (id_user),
	INDEX id_user_id_contact_bloonmail_ab_properties_index (id_user, id_contact),
	INDEX id_contact_prop_type_bloonmail_ab_properties_index (id_contact, prop_type)
...
CREATE TABLE bloonmail_ab_contacts (
...
CREATE INDEX id_user_bloonmail_ab_contacts_index ON bloonmail_ab_contacts (id_user);

CREATE TABLE bloonmail_ab_properties (
...
CREATE INDEX id_user_bloonmail_ab_properties_index ON bloonmail_ab_properties (id_user);
CREATE INDEX id_user_id_contact_bloonmail_ab_properties_index ON bloonmail_ab_properties (id_user, id_contact);
		...
		switch ($this->sDsnType)
		{
			case 'mysql':
				$mCache = $this->dataBaseUpgrade($this->sDsnType.'-ab-version', array(
					...
					2 => array(
'ALTER TABLE bloonmail_ab_properties ADD prop_value_lower varchar(255) NOT NULL DEFAULT \'\' AFTER prop_value_custom;'
					),
					3 => array(
'ALTER TABLE bloonmail_ab_properties CHANGE prop_value prop_value TEXT NOT NULL;',
'ALTER TABLE bloonmail_ab_properties CHANGE prop_value_custom prop_value_custom TEXT NOT NULL;',
'ALTER TABLE bloonmail_ab_properties CHANGE prop_value_lower prop_value_lower TEXT NOT NULL;'
					)
				));
				break;
			case 'pgsql':
				$mCache = $this->dataBaseUpgrade($this->sDsnType.'-ab-version', array(
					1 => $this->getInitialTablesArray($this->sDsnType),
					2 => array(
'ALTER TABLE bloonmail_ab_properties ADD prop_value_lower text NOT NULL DEFAULT \'\';'
					),
					3 => array()
				));
				break;
...
'ALTER TABLE bloonmail_ab_properties ADD prop_value_lower text NOT NULL DEFAULT \'\';'
	...
	private function getContactFreq($iUserID, $iIdContact)
	{
		...
		$sSql = 'SELECT prop_value, prop_frec FROM bloonmail_ab_properties WHERE id_user = :id_user AND id_contact = :id_contact AND prop_type = :type';
		$aParams = array(
			':id_user' => array($iUserID, \PDO::PARAM_INT),
			':id_contact' => array($iIdContact, \PDO::PARAM_INT),
