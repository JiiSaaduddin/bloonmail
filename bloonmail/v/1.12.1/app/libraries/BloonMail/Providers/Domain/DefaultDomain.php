...
class DefaultDomain implements \BloonMail\Providers\Domain\DomainAdminInterface
{
	/**
	 * @var string
	 */
	...
	public function Load($sName, $bFindWithWildCard = false, $bCheckDisabled = true, $bCheckAliases = true)
	{
		...
		{
			$aDomain = \BloonMail\Utils::CustomParseIniFile($this->sDomainPath.'/'.$sRealFileName.'.ini');
...
//			}
			...
			$mResult = \BloonMail\Model\Domain::NewInstanceFromDomainConfigArray($sName, $aDomain);
		}
		...
		else if ($bFindWithWildCard)
		{
			...
			if (0 < \strlen($sNames))
			{
				if (\BloonMail\Plugins\Helper::ValidateWildcardValues(
