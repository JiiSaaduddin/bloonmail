...
class SieveStorage implements \BloonMail\Providers\Filters\FiltersInterface
{
	const NEW_LINE = "\r\n";

	const SIEVE_FILE_NAME = 'bloonmail.user';
	const SIEVE_FILE_NAME_RAW = 'bloonmail.user.raw';
	...
	{
		$oSieveClient = \MailSo\Sieve\ManageSieveClient::NewInstance()->SetLogger($this->oLogger);
		$oSieveClient->SetTimeOuts(10, (int) \BloonMail\Api::Config()->Get('labs', 'sieve_timeout', 10));
	...
	/**
	 * @param \BloonMail\Providers\Filters\Classes\FilterCondition $oCondition
	 *
	 * @return string
	 */
	private function conditionToSieveScript($oCondition, &$aCapa)
	{
		...
		if (0 < \strlen($sValue) ||
			(0 < \strlen($sValue) && 0 < \strlen($sValueSecond) &&
				\BloonMail\Providers\Filters\Enumerations\ConditionField::HEADER === $oCondition->Field()))
		{
			switch ($oCondition->Type())
			{
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::OVER:
					$sTypeWord = ':over';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::UNDER:
					$sTypeWord = ':under';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::NOT_EQUAL_TO:
					$sResult .= 'not ';
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::EQUAL_TO:
					$sTypeWord = ':is';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::NOT_CONTAINS:
					$sResult .= 'not ';
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::CONTAINS:
					$sTypeWord = ':contains';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionType::REGEX:
					$sTypeWord = ':regex';
					$aCapa['regex'] = true;
					break;
				default:
			...
			{
				case \BloonMail\Providers\Filters\Enumerations\ConditionField::FROM:
					$sResult .= 'header '.$sTypeWord.' ["From"]';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionField::RECIPIENT:
					$sResult .= 'header '.$sTypeWord.' ["To", "CC"]';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionField::SUBJECT:
					$sResult .= 'header '.$sTypeWord.' ["Subject"]';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionField::HEADER:
					$sResult .= 'header '.$sTypeWord.' ["'.$this->quote($sValueSecond).'"]';
					break;
				case \BloonMail\Providers\Filters\Enumerations\ConditionField::SIZE:
			...
			if ($bTrue)
			{
				if (\in_array($oCondition->Field(), array(
					\BloonMail\Providers\Filters\Enumerations\ConditionField::FROM,
					\BloonMail\Providers\Filters\Enumerations\ConditionField::RECIPIENT
				)) && false !== \strpos($sValue, ','))
				{
					$self = $this;
				...
				else if (\BloonMail\Providers\Filters\Enumerations\ConditionField::SIZE === $oCondition->Field())
				{
	...
	/**
	 * @param \BloonMail\Providers\Filters\Classes\Filter $oFilter
	 * @param array $aCapa
	 *
	 * @return string
	 */
	...
	{
		$sNL = \BloonMail\Providers\Filters\SieveStorage::NEW_LINE;
		$sTab = '    ';
		...
		if (\is_array($aConditions))
		{
			...
			{
				if (\BloonMail\Providers\Filters\Enumerations\ConditionsType::ANY ===
		...
		if ($oFilter->MarkAsRead() && \in_array($oFilter->ActionType(), array(
			\BloonMail\Providers\Filters\Enumerations\ActionType::NONE,
			\BloonMail\Providers\Filters\Enumerations\ActionType::MOVE_TO,
			\BloonMail\Providers\Filters\Enumerations\ActionType::FORWARD
		)))
		{
		...
		{
			case \BloonMail\Providers\Filters\Enumerations\ActionType::NONE:
				$aResult[] = $sTab.'stop;';
				break;
			case \BloonMail\Providers\Filters\Enumerations\ActionType::DISCARD:
				$aResult[] = $sTab.'discard;';
				$aResult[] = $sTab.'stop;';
				break;
			case \BloonMail\Providers\Filters\Enumerations\ActionType::VACATION:
				...
				break;
			case \BloonMail\Providers\Filters\Enumerations\ActionType::REJECT:
				$sValue = \trim($oFilter->ActionValue());
				if (0 < \strlen($sValue))
				{
					$aCapa['reject'] = true;
				...
				break;
			case \BloonMail\Providers\Filters\Enumerations\ActionType::FORWARD:
				...
				break;
			case \BloonMail\Providers\Filters\Enumerations\ActionType::MOVE_TO:
				$sValue = $oFilter->ActionValue();
				if (0 < \strlen($sValue))
				{
	...
	private function collectionToFileString($aFilters)
	{
		...
		$aParts[] = '# This is BloonMail Webmail sieve script.';
		$aParts[] = '# Please don\'t change anything here.';
		$aParts[] = '# JMDREVOLUTION:SIEVE';
		$aParts[] = '';

		foreach ($aFilters as /* @var $oItem \BloonMail\Providers\Filters\Classes\Filter */ $oItem)
		{
			$aData = array();
			$aData[] = '/*';
	...
	{
		$aResult = array();
		if (!empty($sFileString) && false !== \strpos($sFileString, 'JMDREVOLUTION:SIEVE'))
		{
			$aMatch = array();
				...
				isset($aMatch[2]) && \is_array($aMatch[2]))
			{
				foreach ($aMatch[2] as $sEncodedLine)
				{
					if (!empty($sEncodedLine))
					{
						...
						{
							$oItem = new \BloonMail\Providers\Filters\Classes\Filter();
