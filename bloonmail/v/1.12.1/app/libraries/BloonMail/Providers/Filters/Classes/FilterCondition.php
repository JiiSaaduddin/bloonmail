...
class FilterCondition
{
	...
	{
		$this->sField = \BloonMail\Providers\Filters\Enumerations\ConditionField::FROM;
		$this->sType = \BloonMail\Providers\Filters\Enumerations\ConditionType::EQUAL_TO;
		$this->sValue = '';
		$this->sValueSecond = '';
	}
	...
	public function FromJSON($aData)
	{
		if (\is_array($aData))
		{
			...
			$this->sType = isset($aData['Type']) ? $aData['Type'] :
				\BloonMail\Providers\Filters\Enumerations\ConditionType::EQUAL_TO;
	...
	public static function CollectionFromJSON($aCollection)
	{
		...
		if (\is_array($aCollection) && 0 < \count($aCollection))
		{
			foreach ($aCollection as $aItem)
			{
				...
				{
					$oItem = new \BloonMail\Providers\Filters\Classes\FilterCondition();
