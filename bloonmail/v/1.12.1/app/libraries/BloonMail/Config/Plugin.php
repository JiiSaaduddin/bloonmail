...
class Plugin extends \BloonMail\Config\AbstractConfig
{
	/**
	 * @var array
	 */
	...
	public function __construct($sPluginName, $aMap = array())
	{
		...
		parent::__construct('plugin-'.$sPluginName.'.ini', '; BloonMail Webmail plugin ('.$sPluginName.')');
	}
	...
	private function convertConfigMap($aMap)
	{
		...
		{
			$aResultMap = array();
			foreach ($aMap as /* @var $oProperty \BloonMail\Plugins\Property */ $oProperty)
			{
				if ($oProperty)
				{
