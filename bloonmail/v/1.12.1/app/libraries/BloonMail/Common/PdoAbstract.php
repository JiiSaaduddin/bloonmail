...
//				$this->oLogger->Write('PDO:'.$sPdoType.($bCaseFunc ? '/SQLITE_NOCASE_UTF8' : ''));
	...
	protected function getUserId($sEmail, $bSkipInsert = false, $bCache = true)
	{
		...
		$oStmt = $this->prepareAndExecute('SELECT id_user FROM bloonmail_users WHERE rl_email = :rl_email',
			array(
		...
		if (!$bSkipInsert)
		{
			...
			$oStmt = $this->prepareAndExecute('INSERT INTO bloonmail_users (rl_email) VALUES (:rl_email)',
	...
	protected function getSystemValue($sName, $bReturnIntValue = true)
	{
		...
		if ($oPdo)
		{
			if ($bReturnIntValue)
			{
				$sQuery = 'SELECT value_int FROM bloonmail_system WHERE sys_name = ?';
			}
			else
			{
				$sQuery = 'SELECT value_str FROM bloonmail_system WHERE sys_name = ?';
			}
	...
	protected function setVersion($sName, $iVersion)
	{
		...
		if ($oPdo)
		{
			$sQuery = 'DELETE FROM bloonmail_system WHERE sys_name = ? AND value_int <= ?;';
			$this->writeLog($sQuery);
			...
			if ($bResult)
			{
				$sQuery = 'INSERT INTO bloonmail_system (sys_name, value_int) VALUES (?, ?);';
				$this->writeLog($sQuery);
	...
	protected function initSystemTables()
	{
		...
		if ($oPdo)
		{
			...
			{
				case 'mysql':
					$aQ[] = 'CREATE TABLE IF NOT EXISTS bloonmail_system (
sys_name varchar(50) NOT NULL,
value_int int UNSIGNED NOT NULL DEFAULT 0,
value_str varchar(128) NOT NULL DEFAULT \'\',
INDEX sys_name_bloonmail_system_index (sys_name)
) /*!40000 ENGINE=INNODB *//*!40101 CHARACTER SET utf8 COLLATE utf8_general_ci */;';
					$aQ[] = 'CREATE TABLE IF NOT EXISTS bloonmail_users (
id_user int UNSIGNED NOT NULL AUTO_INCREMENT,
rl_email varchar(128) NOT NULL DEFAULT \'\',
PRIMARY KEY(id_user),
INDEX rl_email_bloonmail_users_index (rl_email)
) /*!40000 ENGINE=INNODB */;';
					break;

				case 'pgsql':
					$aQ[] = 'CREATE TABLE bloonmail_system (
sys_name varchar(50) NOT NULL,
value_int integer NOT NULL DEFAULT 0,
value_str varchar(128) NOT NULL DEFAULT \'\'
);';
					$aQ[] = 'CREATE INDEX sys_name_bloonmail_system_index ON bloonmail_system (sys_name);';
					$aQ[] = 'CREATE SEQUENCE id_user START WITH 1 INCREMENT BY 1 NO MAXVALUE NO MINVALUE CACHE 1;';
					$aQ[] = 'CREATE TABLE bloonmail_users (
id_user integer DEFAULT nextval(\'id_user\'::text) PRIMARY KEY,
rl_email varchar(128) NOT NULL DEFAULT \'\'
);';
					$aQ[] = 'CREATE INDEX rl_email_bloonmail_users_index ON bloonmail_users (rl_email);';
					break;
