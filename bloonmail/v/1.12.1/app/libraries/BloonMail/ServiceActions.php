...
class ServiceActions
{
	 ...
	 * @param array $aPaths
	 *
	 * @return \BloonMail\ServiceActions
	 */
	...
	public function ServiceAjax()
	{
		...
		try
		{
			if ($this->oHttp->IsPost() && !in_array($sAction, array('JsInfo', 'JsError')) &&
				...
				$this->oHttp->GetPost('XToken', '') !== \BloonMail\Utils::GetCsrfToken())
			{
				throw new \BloonMail\Exceptions\ClientException(\BloonMail\Notifications::InvalidToken);
			}
		...
		catch (\Exception $oException)
		{
			...
			if (\is_array($aResponseItem) && $oException instanceof \BloonMail\Exceptions\ClientException)
			{
				if ('Folders' === $sAction)
				{
	...
	public function ServiceOwnCloudAuth()
	{
		...
		if (!\BloonMail\Utils::IsOwnCloud() ||
			!isset($_ENV['___bloonmail_owncloud_email']) ||
			!isset($_ENV['___bloonmail_owncloud_password']) ||
			empty($_ENV['___bloonmail_owncloud_email'])
		)
		{
		...
		$sEmail = $_ENV['___bloonmail_owncloud_email'];
		$sPassword = $_ENV['___bloonmail_owncloud_password'];
		...
		try
		{
			...
			$bLogout = !($oAccount instanceof \BloonMail\Model\Account);
		}
		catch (\Exception $oException)
		{
	...
	private function privateUpload($sAction, $iSizeLimit = 0)
	{
		...
		try
		{
			$aFile = null;
			$sInputName = 'uploader';
			$iError = \BloonMail\Enumerations\UploadError::UNKNOWN;
			...
			if (isset($_FILES, $_FILES[$sInputName], $_FILES[$sInputName]['name'], $_FILES[$sInputName]['tmp_name'], $_FILES[$sInputName]['size']))
			{
				...
				{
					$iError = \BloonMail\Enumerations\UploadError::CONFIG_SIZE;
				}
			...
			{
				$iError = \BloonMail\Enumerations\UploadError::EMPTY_FILES_DATA;
			}
	...
	public function ServiceProxyExternal()
	{
		...
		if (!empty($sData) && $this->oActions->Config()->Get('labs', 'use_local_proxy_for_external_images', false))
		{
			...
			$aData = \BloonMail\Utils::DecodeKeyValuesQ($sData);
			if (\is_array($aData) && !empty($aData['Token']) && !empty($aData['Url']) && $aData['Token'] === \BloonMail\Utils::GetConnectionToken())
			{
				$iCode = 404;
				$sContentType = '';
				$mResult = $this->oHttp->GetUrlAsString($aData['Url'], 'BloonMail External Proxy', $sContentType, $iCode);
	...
	public function ServiceRaw()
	{
		...
		catch (\BloonMail\Exceptions\ClientException $oException)
		{
			...
			{
				case \BloonMail\Notifications::AuthError:
...
//		sleep(2);
		...
		if (!empty($this->aPaths[3]))
		{
			...
			if ($bCacheEnabled)
			{
				$sCacheFileName = \BloonMail\KeyPathHelper::LangCache(
	...
	public function ServiceTemplates()
	{
		...
		if ($bCacheEnabled)
		{
			$sCacheFileName = \BloonMail\KeyPathHelper::TemplatesCache($bAdmin, $this->oActions->Plugins()->Hash());
			$sResult = $this->Cacher()->Get($sCacheFileName);
		}
	...
	public function ServicePlugins()
	{
		...
		if ($bCacheEnabled)
		{
			$sCacheFileName = \BloonMail\KeyPathHelper::PluginsJsCache($this->oActions->Plugins()->Hash());
			$sResult = $this->Cacher()->Get($sCacheFileName);
		}
	...
	public function ServiceCss()
	{
		...
		if (!empty($this->aPaths[4]))
		{
			...
			if ($bCacheEnabled)
			{
				$sCacheFileName = \BloonMail\KeyPathHelper::CssCache($sTheme, $this->oActions->Plugins()->Hash());
				$sResult = $this->Cacher()->Get($sCacheFileName);
			}
			...
			if (0 === \strlen($sResult))
			{
				try
				{
					...
					$oLess = new \BloonMailVendor\lessc();
					...
					{
						$aResult[] = '@base: "'.
							($bCustomTheme ? \BloonMail\Utils::WebPath() : \BloonMail\Utils::WebVersionPath()).
							'themes/'.$sRealTheme.'/";';
	...
	{
		\BloonMail\Utils::SetCookie(\BloonMail\Actions::RL_MOBILE_TYPE, 'mobile');
		$this->oActions->Location('./');
		return '';
	}
	...
	{
		\BloonMail\Utils::SetCookie(\BloonMail\Actions::RL_MOBILE_TYPE, 'desktop');
		$this->oActions->Location('./');
		return '';
	}
	...
	public function ServiceBadBrowser()
	{
		...
		return \strtr(\file_get_contents(APP_VERSION_ROOT_PATH.'app/templates/BadBrowser.html'), array(
			'{{BaseWebStaticPath}}' => \BloonMail\Utils::WebStaticPath(),
			'{{ErrorTitle}}' => $sTitle,
			'{{ErrorHeader}}' => $sTitle,
			'{{ErrorDesc}}' => $sDesc
		));
	...
	public function ServiceSso()
	{
		...
		if (!empty($sSsoHash))
		{
			...
			$sSsoSubData = $this->Cacher()->Get(\BloonMail\KeyPathHelper::SsoCacherKey($sSsoHash));
			if (!empty($sSsoSubData))
			{
				$mData = \BloonMail\Utils::DecodeKeyValuesQ($sSsoSubData);
				$this->Cacher()->Delete(\BloonMail\KeyPathHelper::SsoCacherKey($sSsoHash));
					...
					(0 === $mData['Time'] || \time() - 10 < $mData['Time']))
				{
					...
					try
					{
						...
						if ($oAccount instanceof \BloonMail\Model\Account && $aAdditionalOptions)
						{
		...
		switch (\strtolower($this->oHttp->GetRequest('Output', 'Redirect')))
		{
			case 'json':
				...
				$aResult = array(
					...
					'Result' => $oAccount instanceof \BloonMail\Model\Account ? true : false,
					'ErrorCode' => 0
				);
				...
				if (!$aResult['Result'])
				{
					...
					{
						$aResult['ErrorCode'] = \BloonMail\Notifications::AuthError;
					}
				}
	...
	public function ServiceExternalSso()
	{
			...
			!empty($sKey) && $sKey === \trim($this->oHttp->GetRequest('SsoKey', '')))
		{
			...
			$sResult = \BloonMail\Api::GetUserSsoHash($sEmail, $sPassword);
	...
	private function changeAction()
	{
		...
		if ($oAccount && $this->oActions->GetCapa(false, false, \BloonMail\Enumerations\Capa::ADDITIONAL_ACCOUNTS, $oAccount))
		{
	...
	public function compileTemplates($bAdmin = false, $bJsOutput = true)
	{
		...
		\BloonMail\Utils::CompileTemplates($aTemplates, APP_VERSION_ROOT_PATH.'app/templates/Views/Components', 'Component');
		\BloonMail\Utils::CompileTemplates($aTemplates, APP_VERSION_ROOT_PATH.'app/templates/Views/'.($bAdmin ? 'Admin' : 'User'));
		\BloonMail\Utils::CompileTemplates($aTemplates, APP_VERSION_ROOT_PATH.'app/templates/Views/Common');
		...
		return $bJsOutput ? 'window.bloonmailTEMPLATES='.\MailSo\Base\Utils::Php2js(array($sHtml), $this->Logger()).';' : $sHtml;
	}
	...
	private function compileLanguage($sLanguage, $bAdmin = false, $bWrapByScriptTag = true)
	{
		...
		\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'app/localization/langs.yml', $aResultLang);
		\BloonMail\Utils::ReadAndAddLang(APP_VERSION_ROOT_PATH.'app/localization/'.
		...
		return
			...
			'window.bloonmailI18N='.$sResult.';'.$sMoment.
