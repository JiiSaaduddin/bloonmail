...
class Service
{
	...
	private function __construct()
	{
		...
		$this->oActions = \BloonMail\Api::Actions();

		$this->oServiceActions = new \BloonMail\ServiceActions($this->oHttp, $this->oActions);
	...
	private function localHandle()
	{
		...
		if ($bAdmin && !$this->oActions->Config()->Get('security', 'allow_admin_panel', true))
		{
			echo $this->oServiceActions->ErrorTemplates('Access Denied.',
				'Access to the BloonMail Webmail Admin Panel is not allowed!', true);
		...
		if ($this->oActions->Config()->Get('labs', 'allow_mobile_version', false))
		{
			...
			if ($bIndex)
			{
				$sMobileType = (string) \BloonMail\Utils::GetCookie(\BloonMail\Actions::RL_MOBILE_TYPE, '');
				switch ($sMobileType) {
					default:
						$sMobileType = '';
		...
		if ($bIndex)
		{
			...
			if (!@\is_dir(APP_DATA_FOLDER_PATH) || !@\is_writable(APP_DATA_FOLDER_PATH))
			{
				echo $this->oServiceActions->ErrorTemplates(
					'Permission denied!',
					'BloonMail Webmail cannot access to the data folder "'.APP_DATA_FOLDER_PATH.'"'
				);
...
//				$aTemplateParameters['{{BaseTemplates}}'] = $this->oServiceActions->compileTemplates($bAdmin, false);
				...
				$sResult = \BloonMail\Utils::ClearHtmlOutput($sResult);
...
//			$sResult .= '][session:'.\md5(\BloonMail\Utils::GetShortToken());
	...
	private function indexTemplateParameters($bAdmin = false, $bMobile = false, $bMobileDevice = false)
	{
		...
		$aTemplateParameters['{{BaseHash}}'] = \md5(
			\implode('~', array(
				...
				\BloonMail\Utils::WebVersionPath(),
				APP_VERSION,
			)).
