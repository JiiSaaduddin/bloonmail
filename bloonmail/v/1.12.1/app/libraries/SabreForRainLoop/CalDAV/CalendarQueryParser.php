...
class CalendarQueryParser {
    ...
    public function parse() {
        ...
        if ($filter->length !== 1) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('Only one filter element is allowed');
        }

        $compFilters = $this->parseCompFilters($filter->item(0));
        if (count($compFilters)!==1) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('There must be exactly 1 top-level comp-filter.');
        }

        $this->filters = $compFilters[0];
        $this->requestedProperties = array_keys(\SabreForBloonMail\DAV\XMLUtil::parseProperties($this->dom->firstChild));
    ...
    protected function parseCompFilters(\DOMElement $parentNode) {
        ...
        for($ii=0; $ii < $compFilterNodes->length; $ii++) {
            ...
            if ($compFilter['time-range'] && !in_array($compFilter['name'],array(
                ...
                throw new \SabreForBloonMail\DAV\Exception\BadRequest('The time-range filter is not defined for the ' . $compFilter['name'] . ' component');
            };
    ...
    protected function parseTimeRange(\DOMElement $parentNode) {
        ...
        if (!is_null($start) && !is_null($end) && $end <= $start) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('The end-date must be larger than the start-date in the time-range filter');
        }
    ...
    protected function parseExpand(\DOMElement $parentNode) {
        ...
        if(!$start) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('The "start" attribute is required for the CALDAV:expand element');
        }
        $start = VObject\DateTimeParser::parseDateTime($start);

        $end = $parentNode->getAttribute('end');
        if(!$end) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('The "end" attribute is required for the CALDAV:expand element');
        }

        $end = VObject\DateTimeParser::parseDateTime($end);

        if ($end <= $start) {
            throw new \SabreForBloonMail\DAV\Exception\BadRequest('The end-date must be larger than the start-date in the expand element.');
        }
