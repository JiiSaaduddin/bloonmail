...
namespace SabreForBloonMail\CalDAV;

use SabreForBloonMail\DAV;
use SabreForBloonMail\DAVACL;
use SabreForBloonMail\VObject;
...
class Plugin extends DAV\ServerPlugin {
    ...
    public function initialize(DAV\Server $server) {
        ...
        $server->propertyMap['{' . self::NS_CALDAV . '}supported-calendar-component-set'] = 'SabreForBloonMail\\CalDAV\\Property\\SupportedCalendarComponentSet';
        $server->propertyMap['{' . self::NS_CALDAV . '}schedule-calendar-transp'] = 'SabreForBloonMail\\CalDAV\\Property\\ScheduleCalendarTransp';

        $server->resourceTypeMapping['\\SabreForBloonMail\\CalDAV\\ICalendar'] = '{urn:ietf:params:xml:ns:caldav}calendar';
        $server->resourceTypeMapping['\\SabreForBloonMail\\CalDAV\\Schedule\\IOutbox'] = '{urn:ietf:params:xml:ns:caldav}schedule-outbox';
        $server->resourceTypeMapping['\\SabreForBloonMail\\CalDAV\\Principal\\IProxyRead'] = '{http://calendarserver.org/ns/}calendar-proxy-read';
        $server->resourceTypeMapping['\\SabreForBloonMail\\CalDAV\\Principal\\IProxyWrite'] = '{http://calendarserver.org/ns/}calendar-proxy-write';
        $server->resourceTypeMapping['\\SabreForBloonMail\\CalDAV\\Notifications\\ICollection'] = '{' . self::NS_CALENDARSERVER . '}notification';
    ...
    public function httpMkCalendar($uri) {
        ...
        //    throw new \SabreForBloonMail\DAV\Exception\Forbidden('iCal has major bugs in it\'s RFC3744 support. Therefore we are left with no other choice but disabling this feature.');
        //}
